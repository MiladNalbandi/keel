...
scenario('init: the runbook hands off instead of claiming the project is healthy', (dir) => {
  ...
  return /--by model. is visible as a self-answer|visible as a self-answer/.test(skill) ? null
    : 'the skill does not say a self-answer is visible';
});
...
function huntWithCandidates(dir, candidates) {
  ...
  return keel(['hunt', 'add', '--lens', 'security', '--json', 'cand.json'], dir);
}
...
scenario('hunt: candidates cannot be ingested before the lens set is confirmed', (dir) => {
  ...
  const early = keel(['hunt', 'add', '--lens', 'security', '--json', 'cand.json'], dir);
  ...
  const bypass = keel(['hunt', 'add', '--lens', 'security', '--json', 'cand.json'], dir);
  ...
  const after = keel(['hunt', 'add', '--lens', 'security', '--json', 'cand.json'], dir);
...
scenario('hunt: a malformed candidate is refused by field and index, a fenced one is accepted', (dir) => {
  ...
  const bad = keel(['hunt', 'add', '--lens', 'security', '--json', 'bad.json'], dir);
  ...
  if (!/\[0\]\.where/.test(keel(['hunt', 'add', '--lens', 'security', '--json', 'nowhere.json'], dir).out)) return 'a candidate with no location was not named';
  ...
  const ok = keel(['hunt', 'add', '--lens', 'security', '--json', 'fenced.json'], dir);
...
scenario('hunt: a lens agent cannot set a severity, and the cap is enforced', (dir) => {
  ...
  const run = JSON.parse(fs.readFileSync(path.join(dir, '.keel', 'hunt',
    JSON.parse(fs.readFileSync(path.join(dir, '.keel', 'hunt.json'), 'utf8')).run + '.json'), 'utf8'));
  ...
  const many = Array.from({ length: 13 }, (_, i) => ({ ...CAND, title: `t${i}` }));
  fs.writeFileSync(path.join(dir, 'many.json'), JSON.stringify(many));
  const over = keel(['hunt', 'add', '--lens', 'security', '--json', 'many.json'], dir);
...
scenario('hunt: start repairs the gitignore so the backlog never dirties the tree', (dir) => {
  ...
  keel(['hunt', 'lenses', '--confirm', 'security'], dir);
  fs.writeFileSync(path.join(dir, 'cand.json'), JSON.stringify([CAND]));
  keel(['hunt', 'add', '--lens', 'security', '--json', 'cand.json'], dir);
...
function huntProven(dir, n = 2) {
  const cands = Array.from({ length: n }, (_, i) => ({ ...CAND, title: `finding ${i}`, symptom: `symptom ${i}` }));
  huntWithCandidates(dir, cands);
  keel(['state', 'phase', 'hunt-prove'], dir);
  const ids = [];
  for (let i = 0; i < n; i++) {
    const id = `F-${String(i + 1).padStart(3, '0')}`;
    const rel = path.join('.keel', 'hunt', 'repro', `${id}.sh`);
...
scenario('hunt: a proven verdict without a runnable recipe is refused', (dir) => {
  ...
  if (keel(['hunt', 'prove', 'F-001', '--verdict', 'proven', '--severity', 'high', '--evidence', 'e', '--repro', 'loose.sh'], dir).code === 0) {
    return 'a recipe outside .keel/hunt/repro/ was accepted';
  }
  const rel = path.join('.keel', 'hunt', 'repro', 'F-001.sh');
...
scenario('hunt: candidates hands out one batch at a time', (dir) => {
  ...
  const many = Array.from({ length: 6 }, (_, i) => ({ ...CAND, title: `finding ${i}` }));
...
scenario('hunt: the report refuses to render while a candidate has no verdict', (dir) => {
  huntWithCandidates(dir, [CAND, { ...CAND, title: 'second' }]);
  keel(['state', 'phase', 'hunt-prove'], dir);
  const rel = path.join('.keel', 'hunt', 'repro', 'F-001.sh');
  fs.writeFileSync(path.join(dir, rel), 'curl -fsS localhost:8080/x\n');
  ...
  const md = fs.readFileSync(path.join(dir, 'docs', 'hunts', JSON.parse(
    fs.readFileSync(path.join(dir, '.keel', 'hunt.json'), 'utf8')).run + '.md'), 'utf8');
...
scenario('hunt: the report is byte-identical on a second render', (dir) => {
  ...
  const f = path.join(dir, 'docs', 'hunts', JSON.parse(
    fs.readFileSync(path.join(dir, '.keel', 'hunt.json'), 'utf8')).run + '.md');
...
scenario('hunt: sweep and prove deny every source bucket', (dir) => {
  ...
  if (blocked) return blocked;
  allowed = expectAllowed(hook('pre-tool', edit('.keel/hunt/repro/F-001.sh'), dir));
  ...
  return expectBlocked(hook('pre-tool', edit('.keel/hunt/repro/F-001.spec.ts'), dir), 'hunt-prove');
});
