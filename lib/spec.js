...
function filled(sections, name) {
  ...
  const real = text.split('\n')
    .map((l) => l.trim())
    // A line beginning with `<` is a template instruction. So is one that still carries an
    // unfilled `<token>` anywhere in it — `- As a <role>, I want <capability>` reads as content
    // and says nothing, which is the one shape a placeholder takes inside a list. The token
    // must start with a letter, so a comparison like `len < 3` is left alone.
    .filter((l) => l && !/^</.test(l) && !/<[a-z][a-z0-9 _/|,.-]*>/i.test(l));
...
function check(cfg, spec) {
  ...
  // The sections phase-1 already treats as an exit-criteria failure. `user stories` joins them
  // because criteria with no stated actor are behaviours nobody asked for; `definition of done`
  // because the lines in it keel cannot enforce are exactly the ones skipped when it is absent.
  for (const name of ['user stories', 'validation and security rules', 'data and migrations',
