...
// The machine's project list — what the one dashboard shows. `forget` takes a project off it
// until a session starts there again.
function projectsCmd(args) {
  const projects = require('./projects');
  if (args[0] === 'forget') {
    if (!args[1]) return fail('usage: keel projects forget <name|id|path>');
    const gone = projects.forget(args[1]);
    if (!gone) return fail(`no project "${args[1]}" on the list. \`keel projects\` shows it.`);
    return out(`forgot ${gone.name} (${gone.root}). It comes back the next time a session starts there.`);
  }
  if (args[0] && args[0] !== 'list') return fail(`keel projects: unknown subcommand "${args[0]}"`);
  let here = null;
  try { const cfg = config.load(); if (cfg.configured) here = projects.idOf(cfg.root); } catch (e) { here = null; }
  out(require('../mcp/view').projectsText(here) + `\n\nlist: ${projects.file()}`);
}
...
module.exports = { init, renderConfig, upgrade, stateCmd, commit, gate, escalate, traceCmd, auditCmd, status, verifyCmd, unlock, env,
  ...
  models, stall, projectsCmd, doctorCmd, preflight, archCmd, skillsCmd, coverCmd, memoryCmd, specCmd, boardCmd, todosCmd };
