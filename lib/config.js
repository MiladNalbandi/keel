...
const DEFAULTS = {
  ...
  loops: {
    ...
    red_author: 'main',
    green_author: 'main',
    // `paired` is the loop keel was built around: a red commit, then a green one, with
    // `red-done` between them proving the test failed on an assertion before any code existed.
    // `single` writes the test and the code together and commits once — faster, and nothing then
    // proves the test ever failed, so a tautological assertion has only ship's assertions lens
    // left to catch it. Default stays `paired`; the choice is asked at the start of phase 4.
    commit_style: 'paired',
  ...
  hunt: {
    ...
    // `messaging` is written up in references/lenses.md but left out here: most projects have no
    // broker, and a configured lens with nothing to act on costs a hunter every sweep. Add it in
    // the project config when there is one.
    lenses: ['exploration', 'observability', 'security', 'behavioral', 'technical', 'concurrency',
      'idempotency', 'contract-drift', 'reachability', 'test-integrity', 'data-migration'],
    ...
    lens_lanes: {
      ...
      idempotency: ['api', 'web'],
      'contract-drift': ['both'],
      // Two hunters, each half the target list — the point being speed: reachability's whole
      // brief is a time budget, and splitting the list in half is the cheapest way to keep it.
    ...
    // exploration and observability both earn their place by arriving with proof attached: their
    // candidates are already-reproduced commands, so the prove step that follows is nearly free.
    // observability needs log access — on a fast run with none it will say so and stop, which is
    // the correct outcome and costs one hunter, not the sweep.
