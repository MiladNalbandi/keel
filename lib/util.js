...
const hasKeelConfig = (d) => fs.existsSync(path.join(d, '.keel', 'config.yml'));
const isRepoRoot = (d) => hasKeelConfig(d) || fs.existsSync(path.join(d, '.git'));

function findRepoRoot(start) {
  if (start) return walkUp(start, isRepoRoot) || process.cwd();
  // A lane worktree is a sibling checkout carrying its own .keel/config.yml. Prefer where we
  // actually stand when that is a configured keel project, so a command run inside a lane reads
  // the lane's state and not the session's. Anywhere else, CLAUDE_PROJECT_DIR still wins.
  return walkUp(process.cwd(), hasKeelConfig) ||
    walkUp(process.env.CLAUDE_PROJECT_DIR || process.cwd(), isRepoRoot) || process.cwd();
}
