...
function fast(cfg) {
  ...
  // Outside a repository `changedFiles` is empty for want of git, not because nothing changed — and
  // every diff-scoped tier then passed vacuously while reading like a real one. Say which it was;
  // the comment on hasGit above has always said this and nothing acted on it.
  if (!files.length && !hasGit(cfg)) {
    return { ok: true, vacuous: true, raw: '', fingerprint: null,
      report: 'fast: this directory is not a git repository, so nothing was compared.' };
  }
  const steps = [];
...
function audit(cfg, st, base) {
  ...
  for (const line of log) {
    ...
    // Under `loops.commit_style: single` a criterion is one `feat(AC-n)` commit carrying its test
    // and its code, so this check would flag every commit that mode produces. The separation it
    // protects belongs to the paired loop, and not having it is the mode's stated cost.
    // Config-scoped rather than per-commit: nothing on a commit records which style made it, and
    // inventing a marker to satisfy a check the mode already opts out of would be worse.
    const singleMode = ((cfg.loops || {}).commit_style === 'single');
    if (isCodeCommit && !singleMode && buckets.some((b2) => ['api-test', 'web-test'].includes(b2))) {
