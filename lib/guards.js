...
function checkBash(cfg, state, command) {
  ...
  if (/\.env(\s|$)|cat\s+[^|]*\.env\b/.test(cmd) && /\b(cat|less|head|tail|more|grep)\b/.test(cmd)) {
    return { ok: false, reason: 'Printing .env would put secrets in the conversation. Use `keel env` instead.' };
  }
...
module.exports = { classify, checkEdit, checkRead, checkBash, checkMcp, MATRIX, SERENA_EDIT };
