...
const AGENT_SETTINGS = {
  ...
  'lane-runner': 'model_lane_runner', 'arch-surveyor': 'model_arch_surveyor',
  'security-auditor': 'model_security_auditor', 'dependency-triager': 'model_dependency_triager',
  reproducer: 'model_reproducer',
};
// Must stay in step with `userConfig` in .claude-plugin/plugin.json: an agent missing here
// is invisible to `keel models show` and untouched by `set-all`, so it silently keeps
// whatever model its frontmatter names.
...
function memoryCmd(args = []) {
  ...
  const head = git('rev-parse HEAD', cfg.root).out.trim();
  ...
  if (sub === 'update') {
    if (!fs.existsSync(dir)) return fail('no knowledge base to update. Run `/keel:init` first.', 3);
    ...
    return out([`knowledge verdict written for ${head.slice(0, 7)}.`,
