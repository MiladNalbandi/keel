...
const STATE_CHANGING = [
  ...
  /\bkeel\s+gate\s+\w+/,
  /\bkeel\s+escalate\b/,
  // The mutating hunt subcommands only. `list` and `status` are reads, and a checklist after a
  // read is exactly the noise the rule above is about.
  /\bkeel\s+hunt\s+(start|lenses|add|prove|group|report|close)\b/,
];
