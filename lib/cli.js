...
function huntCmd(args = []) {
  ...
  return fail(`unknown hunt command "${sub}". Use start, lenses, add, candidates, prove, group, report, next, close, resume, list or status.`);
}
...
const AGENT_EFFORTS = { reviewer: 'high', explorer: 'low', investigator: 'high',
  'e2e-author': 'medium', implementer: 'high', 'bulk-reader': 'low',
  'test-author': 'medium', 'setup-doctor': 'medium', 'lane-runner': 'high',
  'arch-surveyor': 'medium', 'security-auditor': 'high', 'dependency-triager': 'medium',
  reproducer: 'medium', hunter: 'high', prover: 'high', librarian: 'high' };
...
function memoryCmd(args = []) {
  ...
  if (sub === 'check') {
    ...
    const stats = r.sections.map((x) => `  ${x.name.padEnd(13)} ${x.missing ? 'missing'
      : `${x.citations} citation(s), ${x.proofs} proof(s)${x.unverified ? `, ${x.unverified} marked unverified` : ''}`}`);
    if (r.pass) return out([`knowledge base: every citation resolves.`, ...stats].join('\n'));
  ...
  if (sub === 'update') {
    ...
    const verdict = { sha: head, content: mem.contentHash(cfg), at: new Date().toISOString(),
      pass: r.pass, sections: SECTIONS.filter((s) => !missing.includes(s)), missing,
      problems: r.problems };
  ...
  return fail(`unknown memory command "${sub}". Use show, reload, check or update.`);
}
