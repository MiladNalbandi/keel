...
function preTool() {
  ...
  if (tool === 'Bash') {
    ...
    if (matchesAnyCommand(String(ti.command || ''), (cfg.coverage || {}).gate_commands)) {
      const cov = readJson(path.join(cfg.root, '.keel', 'coverage.json'), null);
      const head = run('git rev-parse HEAD', { cwd: cfg.root }).out.trim();
      if (!cov) return block('no coverage verdict for this commit. Run `keel verify coverage` first.');
      if (cov.sha !== head) return block(`the coverage verdict is for ${String(cov.sha).slice(0, 7)}, not ${head.slice(0, 7)}. Run \`keel verify coverage\` again.`);
      if (!cov.pass) return block(`coverage is below the threshold (${cov.summary || 'see keel verify coverage'}).`);

      // H17 — dependency gate, on the same commands. It reads a stored verdict and never
      // scans: npm audit makes network calls, far over a hook's budget. And it asks for
      // nothing when no manifest or lockfile changed on this branch, so a change that
      // cannot have introduced a vulnerability is never held up by one.
      if ((cfg.security || {}).enabled !== false && ((cfg.security || {}).pipelines || ['code', 'deps']).includes('deps')) {
        const deps = require('./deps');
        if (deps.manifestsChanged(cfg).length) {
          const sec = readJson(path.join(cfg.root, '.keel', 'security.json'), null);
          if (!sec) return block('a dependency manifest changed on this branch and there is no vulnerability verdict. Run `keel verify deps`.');
          if (sec.sha !== head) return block(`the dependency verdict is for ${String(sec.sha).slice(0, 7)}, not ${head.slice(0, 7)}. Run \`keel verify deps\` again.`);
          if (!sec.pass) return block(`dependency vulnerabilities at or above ${sec.threshold} (${sec.summary}).`);
        }
      }
    }
    return allow();
...
// H5, H6 — format the edited file, reject disabled-test markers
function postTool() {
  const inp = input();
  const { cfg, state } = ctx(inp);
  const ti = inp.tool_input || {};
...
function subagentStart() {
  ...
  const required = {
    'keel:reviewer': 'BLOCKING: yes|no',
    'keel:investigator': 'ROOT-CAUSE: confirmed|unconfirmed',
    'keel:e2e-author': 'E2E-RESULT: pass|fail',
    'keel:implementer': 'GREEN-RESULT: pass|stalled',
    'keel:test-author': 'RED-RESULT: failing|unexpected-pass',
    'keel:lane-runner': 'LANE-RESULT: done|stopped',
    'keel:explorer': 'MAP-END',
    'keel:setup-doctor': 'DIAGNOSIS: fixable|needs-you|unknown',
    'keel:arch-surveyor': 'ARCH: <style> <confidence>',
  }[agent] || 'a single result line';
...
function subagentStop() {
  ...
  const agent = inp.agent_type || inp.subagent_type || '';
  const required = {
    'keel:reviewer': /BLOCKING:\s*(yes|no)/i,
    'keel:investigator': /ROOT-CAUSE:\s*(confirmed|unconfirmed)/i,
    'keel:e2e-author': /E2E-RESULT:\s*(pass|fail)/i,
    'keel:implementer': /GREEN-RESULT:\s*(pass|stalled)/i,
    'keel:arch-surveyor': /ARCH:\s*\w+/i,
    'keel:test-author': /RED-RESULT:\s*(failing|unexpected-pass)/i,
    'keel:lane-runner': /LANE-RESULT:\s*(done|stopped)/i,
    'keel:explorer': /MAP-END/,
    'keel:setup-doctor': /DIAGNOSIS:\s*(fixable|needs-you|unknown)/i,
  }[agent];
  if (!required) return allow();
  const text = inp.last_message || lastAssistant(inp.transcript_path) || '';
  if (required.test(text)) return allow();
  stopBlock(`${agent} must finish with its result line (${String(required).replace(/[/\\^$]|\\s\*/g, '')}). Add it and finish.`);
}
  ...
  try {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (e.type === 'assistant' && content) {
        return (Array.isArray(content) ? content : [content]).map((c) => c.text || '').join('\n');
      }
    }
  } catch (e) { /* ignore */ }
  return '';
...
module.exports = { dispatch, matchesAnyCommand };
