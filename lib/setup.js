...
function rungs(cfg, disc, opts = {}) {
  ...
  const probeOnly = /\s-q\s+help\b/.test(depsCmd);
  const list = [
    ...
    //
    // A fast run drops this rung when it is still the default probe. `./gradlew -q help` proves the
    // build tool starts; `compile` then resolves the same graph and proves strictly more, so the
    // probe is paying a whole cold-daemon start for a subset of the next rung. A project that
    // pointed commands.deps_api at a real resolution task keeps it — that one checks something
    // compile does not.
    { id: 'dependencies', label: /\s-q\s+help\b/.test(depsCmd) ? 'Build tool starts' : 'Backend dependencies',
      cmd: (opts.fast && probeOnly) ? null : depsCmd, cwd: b,
      why: (opts.fast && probeOnly) ? 'skipped by --fast: `compile` resolves the same graph and proves more' : undefined },
...
async function ladder(cfg, opts = {}) {
  ...
  // A fast run re-uses what already passed on this machine rather than proving it again.
  if (opts.fast) opts = Object.assign({}, opts, { resume: true });
  const all = rungs(cfg, disc, opts);
  ...
  return { results, stopped, discovery: disc, fast: !!opts.fast };
}
...
function runbook(cfg, ladderResult) {
  ...
  const text = [
    ...
    `${checked.length} of ${ladderResult.results.length} steps ran and passed; ${notChecked.length} were not checked.`
      + (fast ? ' This was a `--fast` run: it re-used what had already passed here, and dropped the'
        + ' build-tool probe because `compile` proves more than it did.' : ''),
    '',
    '## Prerequisites',
    '',
