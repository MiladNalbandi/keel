...
function sessionStart() {
  ...
  const lines = [
    ...
    (() => {
      try {
        const items = require('./todos').build(state);
...
function postTool() {
  ...
  if (!ti.file_path && !ti.path) {
    ...
    if (!st.active(state) || !todos.isStateChanging(ti.command)) return allow();
    const list = todos.render(state);
    if (!list) return allow();
    // A hook cannot call the todo tool — only the model can. So this injects the current
    // list and says what to do with it; it makes the checklist reliable, not enforced.
    return say([list, '', 'Mirror this into your todo list so the step-by-step view matches the flow.'].join('\n'));
  }
...
function subagentStart() {
  ...
  const required = CONTRACT_LABEL[agent] || 'a single result line';
...
function subagentStop() {
  ...
  // The old message ran the regex through a lossy replace and told the agent to emit
  // `DIAGNOSIS:s*(fixable|...)i` — a pattern that could never match.
  stopBlock(`${agent} must finish with its result line (${CONTRACT_LABEL[agent] || 'a single result line'}). Add it and finish.`);
}
...
module.exports = { dispatch, matchesAnyCommand, nextStep };
