...
const { git, readJson, writeJson } = require('./util');
...
function run(cfg, opts = {}) {
  ...
  if (!results.length) { verdict.pass = false; verdict.problems = ['no coverage reports configured (coverage.reports in .keel/config.yml)']; }
