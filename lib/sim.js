...
// Every session-start registers the project on the machine's list. Scenarios make hundreds of
// throwaway repos, and none of them belong on the user's real dashboard.
if (!process.env.KEEL_SIM_HOME) {
  process.env.KEEL_SIM_HOME = fs.mkdtempSync(path.join(os.tmpdir(), 'keel-home-'));
  process.env.KEEL_HOME = process.env.KEEL_SIM_HOME;
  process.on('exit', () => { try { fs.rmSync(process.env.KEEL_SIM_HOME, { recursive: true, force: true }); } catch (e) { /* gone */ } });
}
...
`);
  ...
  // A real `keel init --write` writes these. Without them keel's own per-machine state rides
  // along in commits, which is what breaks a lane merge.
  w('.gitignore', require('./util').PER_MACHINE_IGNORES.join('\n') + '\n');
...
// One API criterion and two WEB ones, in the api lane: the shape a lane split is offered for.
function startMixed(dir) {
  startRed(dir);
  keel(['state', 'ac', 'AC-004', '--layer', 'WEB'], dir);
  keel(['state', 'ac', 'AC-005', '--layer', 'WEB'], dir);
}
const laneDir = (dir, name = 'web') => path.join(path.dirname(dir), path.basename(dir) + '-' + name);
// Lane worktrees are siblings of the sandbox, so the runner's own cleanup never reaches them.
function dropLane(wt) { try { fs.rmSync(wt, { recursive: true, force: true }); } catch (e) { } }

// A passing release verdict for HEAD, so a scenario about the coverage or deps gate is not
// answered first by the release gate that sits in front of them.
function passRelease(dir) {
  const head = sh('git rev-parse HEAD', dir).out.trim();
  fs.writeFileSync(path.join(dir, '.keel', 'release.json'),
    JSON.stringify({ sha: head, at: new Date().toISOString(), pass: true, summary: 'sim' }));
}
...
scenario('push with a stale coverage verdict is blocked', (dir) => {
  passRelease(dir);   // this scenario is about another gate
  startRed(dir);
...
scenario('coverage verdict is written for HEAD and unblocks push', (dir) => {
  ...
  startRed(dir);
  passRelease(dir);   // after the last commit: this scenario is about the coverage gate
  return expectAllowed(hook('pre-tool', bash('git push -u origin HEAD'), dir));
});
...
scenario('lane start creates a worktree with its own branch, ports and ACs', (dir) => {
  startMixed(dir);
  ...
  if (!/AC-004, AC-005/.test(r.out)) return `the WEB ACs were not handed over: ${r.out.trim()}`;
  const s = keel(['lane', 'status'], dir);
  dropLane(laneDir(dir));
  return /web: open \(background\)/.test(s.out) ? null : `lane not recorded: ${s.out.trim()}`;
});

scenario('lane start needs a flow and some ACs of its own', (dir) => {
  const none = keel(['lane', 'start', 'web'], dir);
  if (expectFails(none, 'no active flow')) return expectFails(none, 'no active flow');
  startRed(dir);                                     // one API AC, nothing for the web lane
  return expectFails(keel(['lane', 'start', 'web'], dir), 'no web acceptance criteria');
});

scenario('lane start seeds the worktree with its own state', (dir) => {
  startMixed(dir);
  keel(['lane', 'start', 'web', '--background'], dir);
  const wt = laneDir(dir);
  const lane = JSON.parse(fs.readFileSync(path.join(wt, '.keel/state.json'), 'utf8'));
  dropLane(wt);
  if (lane.lane !== 'web') return `lane state says lane ${lane.lane}`;
  if (lane.phase !== 'red') return `lane state says phase ${lane.phase}`;
  if (lane.current !== 'AC-004') return `lane current is ${lane.current}`;
  if (Object.keys(lane.acs).sort().join(',') !== 'AC-004,AC-005') return `lane got ${Object.keys(lane.acs)}`;
  if (lane.gates.skipped.web !== 'rest-of-lane') return 'a background lane did not have its gates skipped';
  const parent = JSON.parse(fs.readFileSync(path.join(dir, '.keel/state.json'), 'utf8'));
  return parent.acs['AC-004'].status === 'lane' ? null : `parent did not hand over AC-004: ${parent.acs['AC-004'].status}`;
});

scenario('lane merge brings the branch back and folds its work in', (dir) => {
  startMixed(dir);
  ...
  for (const ac of ['AC-004', 'AC-005']) keel(['state', 'ac', ac, '--layer', 'WEB', '--status', 'done'], wt);
  const r = keel(['lane', 'merge', 'web'], dir);
  if (r.code !== 0) { dropLane(wt); return `merge failed: ${r.out.trim()}`; }
  if (!fs.existsSync(path.join(dir, 'apps/web/src/Extra.tsx'))) return 'the lane commit did not arrive';
  const parent = JSON.parse(fs.readFileSync(path.join(dir, '.keel/state.json'), 'utf8'));
  return parent.acs['AC-004'].status === 'done' ? null : `the lane's work was not folded back: ${parent.acs['AC-004'].status}`;
});

scenario('lane merge refuses to leave unfinished ACs behind', (dir) => {
  startMixed(dir);
  keel(['lane', 'start', 'web'], dir);
  const wt = laneDir(dir);
  const r = keel(['lane', 'merge', 'web'], dir);
  if (expectFails(r, 'AC-004')) { dropLane(wt); return expectFails(r, 'AC-004'); }
  const forced = keel(['lane', 'merge', 'web', '--force'], dir);
  dropLane(wt);
  if (forced.code !== 0) return `forced merge failed: ${forced.out.trim()}`;
  const parent = JSON.parse(fs.readFileSync(path.join(dir, '.keel/state.json'), 'utf8'));
  return parent.acs['AC-004'].status === 'todo' ? null : `the unfinished AC did not come back: ${parent.acs['AC-004'].status}`;
});

scenario('a lane can be started again after it merged', (dir) => {
  startMixed(dir);
  keel(['lane', 'start', 'web'], dir);
  const wt = laneDir(dir);
  for (const ac of ['AC-004', 'AC-005']) keel(['state', 'ac', ac, '--layer', 'WEB', '--status', 'done'], wt);
  if (keel(['lane', 'merge', 'web'], dir).code !== 0) { dropLane(wt); return 'the first merge failed'; }
  keel(['state', 'ac', 'AC-004', '--layer', 'WEB', '--status', 'todo'], dir);
  const again = keel(['lane', 'start', 'web'], dir);
  dropLane(wt);
  return again.code === 0 ? null : `the lane name was not freed by the merge: ${again.out.trim()}`;
});

scenario('gate approve waits for the lane instead of jumping to integration', (dir) => {
  startMixed(dir);
  keel(['lane', 'start', 'web', '--background'], dir);
  const wt = laneDir(dir);
  const r = keel(['gate', 'ac', 'approve'], dir);
  dropLane(wt);
  if (r.code !== 0) return `gate approve failed: ${r.out.trim()}`;
  if (!/lane merge web/.test(r.out)) return `no pointer back to the lane: ${r.out.trim()}`;
  const s = JSON.parse(fs.readFileSync(path.join(dir, '.keel/state.json'), 'utf8'));
  return s.phase === 'gate' ? null : `phase ran ahead to ${s.phase} with the lane still open`;
});

scenario('a WEB criterion runs the web tests and may edit its own test', (dir) => {
  startMixed(dir);
  keel(['state', 'ac', 'AC-004', '--layer', 'WEB', '--current'], dir);
  // A default flow never leaves lane 'api', so this used to be refused as a lane violation.
  const own = hook('pre-tool', edit('apps/web/src/BookmarkForm.test.tsx'), dir);
  if (own.code !== 0) return `a WEB AC could not edit its own test: ${own.out.trim()}`;
  setSim(dir, 'gradle', 'assert');                   // the api build would fail if it were used
  const ok = keel(['verify', 'ac', 'AC-004'], dir);
  if (ok.code !== 0) return `a WEB AC was verified with the api build: ${ok.out.trim()}`;
  setSim(dir, 'gradle', 'pass');
  setSim(dir, 'vitest', 'assert');                   // and the web one is what it really runs
  return expectFails(keel(['verify', 'ac', 'AC-004'], dir));
});
...
scenario('the deps gate blocks a push when a manifest changed with no verdict', (dir) => {
  ...
  passRelease(dir);   // same reason, for the gate that now sits in front of both
...
scenario('review-fix can return to security, for the phase 6.6 full-diff review loop', (dir) => {
  keel(['state', 'start', 'feature', '--spec', 'specs/001-bookmarks.md', '--phase', 'integration'], dir);
  keel(['state', 'phase', 'security'], dir);
  const toFix = keel(['state', 'phase', 'review-fix'], dir);
  if (toFix.code !== 0) return `security -> review-fix should be legal: ${toFix.out.trim()}`;
  const back = keel(['state', 'phase', 'security'], dir);
  if (back.code !== 0) return `review-fix -> security should be legal: ${back.out.trim()}`;
  // And from there e2e is still reachable, so the gate before E2E can proceed once
  // code-reviewer passes.
  const onward = keel(['state', 'phase', 'e2e'], dir);
  return onward.code === 0 ? null : `security -> e2e should still be legal after the loop: ${onward.out.trim()}`;
});

scenario('code-reviewer has its own CODE-REVIEW result line, not BLOCKING or AC-REVIEW', () => {
  const rd = (p) => fs.readFileSync(path.join(__dirname, '..', p), 'utf8');
  const agent = rd('agents/code-reviewer.md');
  if (/BLOCKING:/.test(agent)) return 'code-reviewer must not reuse the BLOCKING label';
  if (/AC-REVIEW:/.test(agent)) return 'code-reviewer must not reuse the AC-REVIEW label';
  return /CODE-REVIEW:\s*pass/.test(agent) ? null : 'code-reviewer declares no CODE-REVIEW result line';
});
...
scenario('memory: update refuses until the sections are chosen', (dir) => {
  // The gate. Five librarians is the most expensive thing init does, and nothing used to ask
  // which of them you wanted — `update` scaffolded and stamped all five without a question.
  const r = keel(['memory', 'update'], dir);
  if (r.code !== 3) return `update ran with no selection: ${r.out.trim()}`;
  if (!/which parts of the knowledge base/.test(r.out)) return `refused without asking: ${r.out.trim()}`;
  for (const n of ['architecture', 'domain', 'conventions', 'data', 'integrations']) {
    if (!r.out.includes(n)) return `the menu omits ${n}: ${r.out.trim()}`;
  }
  // Answering it releases the refusal.
  keel(['memory', 'sections', '--confirm', 'conventions', '--by', 'user'], dir);
  const after = keel(['memory', 'update'], dir);
  return /which parts of the knowledge base/.test(after.out)
    ? `still refused after answering: ${after.out.trim()}` : null;
});

scenario('memory: an unchosen section is not a missing one', (dir) => {
  // The reason the gate could not be built before: `check` read every absent section as missing,
  // and a failing knowledge verdict blocks every push — so choosing two of five cost you the push.
  keel(['memory', 'sections', '--confirm', 'conventions,data', '--by', 'user'], dir);
  const d = path.join(dir, 'docs', 'knowledge');
  fs.mkdirSync(d, { recursive: true });
  fs.writeFileSync(path.join(d, 'conventions.md'), `# Conventions\n\nSee \`${KB_CTRL}:1\`.\n`);
  fs.writeFileSync(path.join(d, 'data.md'), `# Data\n\nSee \`${KB_CTRL}:1\`.\n`);

  const r = keel(['memory', 'check'], dir);
  if (r.code !== 0) return `a deliberately partial knowledge base failed the check: ${r.out.trim()}`;
  if (!/architecture\s+not selected/.test(r.out)) return `an unchosen section did not read "not selected": ${r.out.trim()}`;

  keel(['memory', 'update'], dir);
  const v = JSON.parse(fs.readFileSync(path.join(dir, '.keel', 'memory.json'), 'utf8'));
  if (v.pass !== true) return `the verdict failed on a partial base: ${JSON.stringify(v.problems)}`;
  if ((v.missing || []).length) return `an unchosen section was recorded as missing: ${v.missing.join(', ')}`;
  // And the push gate agrees.
  const push = hook('pre-tool', bash('git push'), dir);
  return /knowledge/.test(push.out) ? `a partial base blocked the push: ${push.out.trim()}` : null;
});

scenario('memory: a section on disk is checked whether or not it was chosen', (dir) => {
  // Selection decides what must exist. It must never decide what gets checked: a file that is
  // there is read back as project authority, so an unsourced one is a problem either way.
  keel(['memory', 'sections', '--confirm', 'conventions', '--by', 'user'], dir);
  const d = path.join(dir, 'docs', 'knowledge');
  fs.mkdirSync(d, { recursive: true });
  fs.writeFileSync(path.join(d, 'conventions.md'), `# Conventions\n\nSee \`${KB_CTRL}:1\`.\n`);
  if (keel(['memory', 'check'], dir).code !== 0) return 'a valid single-section base was refused';
  fs.writeFileSync(path.join(d, 'domain.md'), '# Domain\n\nNo citations here at all.\n');
  const r = keel(['memory', 'check'], dir);
  return r.code !== 0 && /no citations at all/.test(r.out)
    ? null : `an unchosen but present section escaped the check: ${r.out.trim()}`;
});

scenario('memory: confirming again adds sections and keeps what is built', (dir) => {
  // "Re-run init to finish it" — the second answer must add to the first, and must never orphan
  ...
  const r = keel(['memory', 'sections', '--confirm', 'data', '--by', 'user'], dir);
  if (r.code !== 0) return `confirming more failed: ${r.out.trim()}`;
  const v = JSON.parse(fs.readFileSync(path.join(dir, '.keel', 'memory.json'), 'utf8'));
  for (const n of ['conventions', 'data']) {
    if (!(v.selected || []).includes(n)) return `${n} missing from the union: ${JSON.stringify(v.selected)}`;
  }
  if (!/already built:\s+conventions/.test(r.out)) return `a built section was not reported as built: ${r.out.trim()}`;
  if (!/still to write: data/.test(r.out)) return `the remaining work was not named: ${r.out.trim()}`;
  // --none cannot orphan a section that exists.
  const none = keel(['memory', 'sections', '--none'], dir);
  return none.code === 3 && /already exist/.test(none.out)
    ? null : `--none silently dropped a built section: ${none.out.trim()}`;
});
...
function huntWithCandidates(dir, candidates) {
  keel(['hunt', 'start', '--auto', '--scope', 'all'], dir);
...
scenario('hunt: start repairs the gitignore so the backlog never dirties the tree', (dir) => {
  const started = keel(['hunt', 'start', '--auto'], dir);
...
scenario('autonomy: a hunt will not start until you say how closely to watch it', (dir) => {
  const bare = keel(['hunt', 'start'], dir);
  if (bare.code === 0) return 'a hunt started without an autonomy choice';
  for (const want of ['--semi', '--auto', 'nothing is fixed inside a hunt']) {
    if (!bare.out.includes(want)) return `the refusal omits "${want}": ${bare.out.trim()}`;
  }
  // Refusing to start IS the gate — deliberately not a `keel ask` blocking question, which would
  // also block ladder, init --write, memory update and preflight.
  if (fs.existsSync(path.join(dir, '.keel', 'hunt.json'))) return 'a run was created before the choice was made';
  if (keel(['hunt', 'start', '--auto', '--semi'], dir).code === 0) return 'both modes at once was accepted';
  return keel(['hunt', 'start', '--semi'], dir).code === 0 ? null : 'it would not start with --semi';
});

scenario('autonomy: --semi stops at every phase, and each gate releases exactly one thing', (dir) => {
  keel(['hunt', 'start', '--semi'], dir);
  keel(['hunt', 'lenses', '--confirm', 'security', '--by', 'user'], dir);
  fs.writeFileSync(path.join(dir, 'c.json'), JSON.stringify([candAt(1)]));
  keel(['hunt', 'add', '--lens', 'security', '--lane', 'api', '--json', 'c.json'], dir);

  // sweep gate: no candidates handed out, no verdicts recorded
  const early = keel(['hunt', 'candidates', '--batch'], dir);
  if (early.code === 0) return 'candidates were handed out before the sweep gate';
  if (!/sweep gate/.test(early.out)) return `the refusal does not name the gate: ${early.out.trim()}`;
  if (!/--auto/.test(early.out)) return 'the refusal does not say how to avoid gates entirely';
  keel(['state', 'phase', 'hunt-prove'], dir);
  if (keel(['hunt', 'prove', 'F-001', '--verdict', 'unproven', '--evidence', 'e'], dir).code === 0) {
    return 'a verdict was recorded before the sweep gate';
  }
  // The candidate page is never gated: it is what you read in order to decide the sweep gate.
  if (keel(['hunt', 'report', '--candidates'], dir).code !== 0) return 'the candidate page was gated, which would deadlock';

  keel(['hunt', 'gate', 'sweep', 'approve', '--by', 'user'], dir);
  if (keel(['hunt', 'candidates', '--batch'], dir).code !== 0) return 'the sweep gate did not release candidates';
  keel(['hunt', 'prove', 'F-001', '--verdict', 'unproven', '--evidence', 'could not reproduce'], dir);

  // prove gate: no report
  keel(['state', 'phase', 'hunt-report'], dir);
  const noReport = keel(['hunt', 'report'], dir);
  if (noReport.code === 0) return 'the report rendered before the prove gate';
  if (!/prove gate/.test(noReport.out)) return `wrong gate named: ${noReport.out.trim()}`;
  keel(['hunt', 'gate', 'prove', 'approve', '--by', 'user'], dir);
  if (keel(['hunt', 'report'], dir).code !== 0) return 'the prove gate did not release the report';

  // report gate: no handover
  sh('git add -A && git commit -q -m "hunt report"', dir);
  keel(['state', 'phase', 'hunt-triage'], dir);
  const noNext = keel(['hunt', 'next'], dir);
  if (noNext.code === 0) return 'a finding was handed over before the report gate';
  if (!/report gate/.test(noNext.out)) return `wrong gate named: ${noNext.out.trim()}`;
  keel(['hunt', 'gate', 'report', 'approve', '--by', 'user'], dir);
  return keel(['hunt', 'next'], dir).code === 0 ? null : 'the report gate did not release the handover';
});

scenario('autonomy: --auto stops at nothing, and the record says the model approved it', (dir) => {
  keel(['hunt', 'start', '--auto'], dir);
  keel(['hunt', 'lenses', '--confirm', 'security'], dir);
  fs.writeFileSync(path.join(dir, 'c.json'), JSON.stringify([candAt(1)]));
  keel(['hunt', 'add', '--lens', 'security', '--lane', 'api', '--json', 'c.json'], dir);
  if (keel(['hunt', 'candidates', '--batch'], dir).code !== 0) return '--auto stopped at the sweep gate';

  const run = JSON.parse(fs.readFileSync(path.join(dir, '.keel', 'hunt', runIdOf(dir), 'run.json'), 'utf8'));
  // The record has the same shape either way; what differs is who is named.
  for (const g of require('./hunt').GATES) {
    const x = (run.gates || {})[g];
    if (!x || x.decision !== 'approve') return `gate ${g} was not pre-approved under --auto`;
    if (x.by !== 'model') return `gate ${g} claims a human approved it: ${x.by}`;
  }
  if (run.lenses.confirmed_by !== 'model') return 'an --auto lens confirm was not attributed to the model';

  keel(['state', 'phase', 'hunt-prove'], dir);
  keel(['hunt', 'prove', 'F-001', '--verdict', 'unproven', '--evidence', 'e'], dir);
  keel(['state', 'phase', 'hunt-report'], dir);
  keel(['hunt', 'report'], dir);
  const md = fs.readFileSync(path.join(reportDirOf(dir), 'report.md'), 'utf8');
  // A reader has to be able to tell that nobody chose what this hunt looked for.
  if (!/confirmed by the model, not reviewed/.test(md)) return 'the report hides that the lens set was unreviewed';
  return /gates were approved by the model/.test(md) ? null : 'the report hides which gates no human saw';
});

scenario('autonomy: resume names the gate it is waiting on', (dir) => {
  keel(['hunt', 'start', '--semi'], dir);
  keel(['hunt', 'lenses', '--confirm', 'security', '--by', 'user'], dir);
  fs.writeFileSync(path.join(dir, 'c.json'), JSON.stringify([candAt(1)]));
  keel(['hunt', 'add', '--lens', 'security', '--lane', 'api', '--json', 'c.json'], dir);
  const r = keel(['hunt', 'resume'], dir);
  if (!/gate sweep approve/.test(r.out)) return `resume does not name the waiting gate: ${r.out.trim()}`;
  if (!/sweep:waiting/.test(r.out)) return 'resume does not show the gate state';
  keel(['hunt', 'gate', 'sweep', 'approve', '--by', 'user'], dir);
  const after = keel(['hunt', 'resume'], dir);
  return /gate prove approve/.test(after.out) ? null : `resume did not move to the next gate: ${after.out.trim()}`;
});
...
scenario('fast: a fast hunt narrows the lenses and never the proof bar', (dir) => {
  const started = keel(['hunt', 'start', '--auto', '--fast'], dir);
  if (started.code !== 0) return `hunt start --fast failed: ${started.out.trim()}`;
  // The banner names the autonomy and the narrowing together, e.g. "(--auto, --fast)".
  if (!/--fast\)/.test(started.out)) return `the start banner does not say it was fast: ${started.out.split('\n')[0]}`;
...
scenario('every configured lens has a brief in lenses.md, and the reachability script exists', () => {
  const root = path.join(__dirname, '..');
  const all = require('./config').DEFAULTS.hunt.lenses;
  const md = fs.readFileSync(path.join(root, 'skills', 'hunt', 'references', 'lenses.md'), 'utf8');
  const missing = all.filter((l) => !new RegExp(`^## ${l}$`, 'm').test(md));
  if (missing.length) return `no brief in lenses.md for: ${missing.join(', ')}`;
  const script = path.join(root, 'skills', 'hunt', 'references', 'reachability-probe.sh');
  if (!fs.existsSync(script)) return 'reachability-probe.sh is missing';
  // Executable, not just present — a lens whose whole technique is a script that cannot run is
  // a brief with nothing behind it.
  const mode = fs.statSync(script).mode;
  return (mode & 0o111) ? null : 'reachability-probe.sh is not executable';
});

scenario('reachability-probe.sh: ok on a 200, FAIL and nonzero on a timeout', (dir) => {
  const script = path.join(__dirname, '..', 'skills', 'hunt', 'references', 'reachability-probe.sh');
  const targets = path.join(dir, 'targets.txt');
  // Port 1 is reserved and nothing answers there — the same unreachable-target shape a real
  // probe hits, without depending on network access the sandbox may not have.
  fs.writeFileSync(targets, '# a comment, skipped\n\nGET http://127.0.0.1:1/nope\n');
  const r = sh(`bash "${script}" "${targets}" 1`);
  if (r.code === 0) return 'a timed-out target should fail the script';
  if (!/^FAIL\s+GET/m.test(r.out)) return `expected a FAIL line: ${r.out.trim()}`;
  return /elapsed/.test(r.out) ? null : 'no elapsed-time line printed';
});
...
scenario('hunt: two hunts in one day do not overwrite each other, and the report stands alone', (dir) => {
  ...
  keel(['hunt', 'start', '--auto', '--new', '--scope', 'all'], dir);
  keel(['hunt', 'lenses', '--confirm', 'security,behavioral'], dir);
  fs.writeFileSync(path.join(dir, 'cand.json'), JSON.stringify([CAND]));
  keel(['hunt', 'add', '--lens', 'security', '--lane', 'api', '--json', 'cand.json'], dir);
...
/* ------------------------------------- 0.36–0.47: the guards added since

   Everything below shipped without a scenario. That is how a routing bug introduced in 0.16
   survived until a review found it in 0.45. One per enforced behaviour. */

scenario('dependency: a bare install is allowed, a named package is not', (dir) => {
  startRed(dir);
  const bare = hook('pre-tool', bash('npm ci'), dir);
  if (bare.code !== 0) return `a lockfile restore must be allowed: ${bare.out.trim()}`;
  const named = hook('pre-tool', bash('npm install zod'), dir);
  return named.code === 2 ? null : `adding a package should be blocked: ${named.out.trim()}`;
});

scenario('dependency: --prefix is not a package, and a flag cannot hide the subcommand', (dir) => {
  startRed(dir);
  const fp = hook('pre-tool', bash('npm install --prefix web'), dir);
  if (fp.code !== 0) return `--prefix with no package must be allowed: ${fp.out.trim()}`;
  const hidden = hook('pre-tool', bash('npm --prefix web install zod'), dir);
  return hidden.code === 2 ? null : 'a flag before the subcommand must not bypass the guard';
});

scenario('dependency: an approved package installs, and only that one', (dir) => {
  startRed(dir);
  keel(['state', 'dep', 'zod', '--by', 'user'], dir);
  const ok = hook('pre-tool', bash('npm install zod'), dir);
  if (ok.code !== 0) return `an approved dependency must install: ${ok.out.trim()}`;
  const other = hook('pre-tool', bash('npm install lodash'), dir);
  return other.code === 2 ? null : 'the allowlist must be per package';
});

scenario('dependency: a manifest line is caught at commit', (dir) => {
  startRed(dir);
  keel(['state', 'phase', 'green'], dir);
  const gradle = path.join(dir, 'apps/api/build.gradle.kts');
  fs.mkdirSync(path.dirname(gradle), { recursive: true });
  fs.writeFileSync(gradle, 'dependencies {\n    implementation("io.ktor:ktor-client:2.3.0")\n}\n');
  const r = keel(['commit', 'green', 'AC-001', 'adds a client'], dir);
  return r.code !== 0 && /dependenc/i.test(r.out)
    ? null : `editing the build file must not be a way around the guard: ${r.out.trim()}`;
});

scenario('a fix commit keeps green and does not move a repair phase', (dir) => {
  startRed(dir);
  const f = path.join(dir, '.keel', 'state.json');
  const s = JSON.parse(fs.readFileSync(f, 'utf8'));
  s.phase = 'ship';
  s.acs['AC-001'] = Object.assign({}, s.acs['AC-001'], { green: 'abc1234', status: 'done' });
  fs.writeFileSync(f, JSON.stringify(s));
  const src = path.join(dir, 'apps/api/src/main/kotlin/Fix.kt');
  fs.mkdirSync(path.dirname(src), { recursive: true });
  fs.writeFileSync(src, 'fun fix() {}\n');
  keel(['commit', 'fix', 'AC-001', 'review — narrow the query'], dir);
  const after = JSON.parse(fs.readFileSync(f, 'utf8'));
  if (after.phase !== 'ship') return `a ship fix must stay in ship, got "${after.phase}"`;
  if (after.acs['AC-001'].green !== 'abc1234') return 'a fix must not overwrite the green sha';
  return (after.acs['AC-001'].fixes || []).length ? null : 'the fix should be recorded in fixes[]';
});

scenario('refactor: production code yes, tests no, and it cannot reach red', (dir) => {
  startRed(dir);
  keel(['state', 'phase', 'green'], dir);
  const to = keel(['state', 'phase', 'refactor'], dir);
  if (to.code !== 0) return `green -> refactor must be legal: ${to.out.trim()}`;
  const prod = hook('pre-tool', { tool_name: 'Edit', tool_input: { file_path: path.join(dir, 'apps/api/src/main/kotlin/A.kt') } }, dir);
  if (prod.code !== 0) return 'refactor must allow production code';
  const test = hook('pre-tool', { tool_name: 'Edit', tool_input: { file_path: path.join(dir, 'apps/api/src/test/kotlin/ATest.kt') } }, dir);
  if (test.code !== 2) return 'refactor must keep tests frozen';
  return keel(['state', 'phase', 'red'], dir).code !== 0 ? null : 'refactor must not reach red';
});

scenario('the spec is frozen in the loop and still reachable from it', (dir) => {
  startRed(dir);
  const spec = hook('pre-tool', { tool_name: 'Edit', tool_input: { file_path: path.join(dir, 'specs/001-bookmarks.md') } }, dir);
  if (spec.code !== 2) return 'specs/ must be frozen in red';
  return keel(['state', 'phase', 'spec'], dir).code === 0
    ? null : 'red -> spec must be legal, or an amendment has no route';
});

scenario('a secret is refused at commit, not only at the edit', (dir) => {
  startRed(dir);
  keel(['state', 'phase', 'green'], dir);
  const f = path.join(dir, 'apps/api/src/main/kotlin/Key.kt');
  fs.mkdirSync(path.dirname(f), { recursive: true });
  // A fixture, and keel's own hook blocks this file otherwise — which is the rule working. keel:allow-secret
  fs.writeFileSync(f, 'val k = "AKIA' + 'IOSFODNN7EXAMPLE"\n');
  const r = keel(['commit', 'green', 'AC-001', 'adds a key'], dir);
  return r.code !== 0 && /secret/i.test(r.out) ? null : `a staged secret must refuse: ${r.out.trim()}`;
});

scenario('knowledge: never-asked is not the same as missing', (dir) => {
  fs.mkdirSync(path.join(dir, 'docs', 'knowledge'), { recursive: true });
  // No selection recorded — what a fast init leaves, and a legitimate state.
  const unanswered = keel(['memory', 'check'], dir);
  if (unanswered.code !== 0) return `an unanswered gate must not read as a defect: ${unanswered.out.trim()}`;
  if (!/not chosen yet/.test(unanswered.out)) return `should say not chosen yet: ${unanswered.out.trim()}`;
  // The gate keeps its teeth: nothing is built until somebody answers.
  if (keel(['memory', 'update'], dir).code === 0) return 'memory update must refuse while unanswered';
  // Answered with none: passes, and says why.
  keel(['memory', 'sections', '--none', '--by', 'user'], dir);
  const none = keel(['memory', 'check'], dir);
  if (none.code !== 0 || !/not selected/.test(none.out)) return `--none must pass: ${none.out.trim()}`;
  // Chosen but absent is still a real problem — the case this must not weaken.
  keel(['memory', 'sections', '--confirm', 'conventions', '--by', 'user'], dir);
  return keel(['memory', 'check'], dir).code !== 0
    ? null : 'a chosen section that does not exist must still fail';
});

scenario('the AC gate points at ac-reviewer, and ship still points at reviewer', (dir) => {
  const root = path.join(__dirname, '..');
  const rd = (p) => fs.readFileSync(path.join(root, p), 'utf8');
  // Both agents exist and are distinct: this must not have been done by editing keel:reviewer.
  if (!fs.existsSync(path.join(root, 'agents/ac-reviewer.md'))) return 'agents/ac-reviewer.md is missing';
  const ship = rd('skills/ship/SKILL.md');
  if (!/one lens each from `review\.lenses`/.test(ship)) return 'ship lost its per-lens dispatch';
  if (/keel:ac-reviewer/.test(ship)) return 'ship must keep using keel:reviewer over the branch';
  // The AC-scoped sites — both the skill and the two strings the CLI prints — name the new one.
  const loop = rd('skills/feature/references/ac-loop.md');
  if (/Run `keel:reviewer`/.test(loop)) return 'the AC gate still dispatches keel:reviewer';
  if (!/keel:ac-reviewer/.test(loop)) return 'ac-loop.md does not name keel:ac-reviewer';
  const cli = rd('lib/cli.js');
  if (/run keel:reviewer on this AC|keel:reviewer subagent on this AC/.test(cli)) {
    return 'a CLI string still sends the AC gate to keel:reviewer';
  }
  // And its verdict is its own, so the board can never read it as ship's.
  const agent = rd('agents/ac-reviewer.md');
  if (/BLOCKING:/.test(agent)) return 'ac-reviewer must not reuse the BLOCKING label';
  return /AC-REVIEW:\s*pass/.test(agent) ? null : 'ac-reviewer declares no AC-REVIEW result line';
});

// /keel:review starts the flow's own reviewers by hand. It is only worth having if it hands them
// the same lenses and reads back the same verdict lines the flow does.
scenario('/keel:review names every lens, agent and verdict line the flow uses', () => {
  const root = path.join(__dirname, '..');
  const rd = (f) => fs.readFileSync(path.join(root, f), 'utf8');
  const skill = rd('skills/review/SKILL.md');
  const hint = (skill.match(/^argument-hint:\s*"(.*)"$/m) || [])[1] || '';
  const problems = [];
  const { CONTRACTS } = require('./hooks');
  for (const agent of ['keel:code-reviewer', 'keel:reviewer', 'keel:ac-reviewer']) {
    if (!fs.existsSync(path.join(root, 'agents', agent.split(':')[1] + '.md'))) problems.push(`no agent file for ${agent}`);
    if (!skill.includes('`' + agent + '`')) problems.push(`the skill never names ${agent}`);
    const label = CONTRACTS[agent] && CONTRACTS[agent].label;
    if (!label) { problems.push(`hooks has no verdict contract for ${agent}`); continue; }
    if (!skill.includes(label)) problems.push(`the skill does not read back "${label}"`);
  }
  // Every lens the reviewer agent knows must be an argument, and the default set must be named.
  const lenses = [...rd('agents/reviewer.md').matchAll(/for the (\w+) lens/g)].map((m) => m[1]);
  if (!lenses.length) return 'could not read the lenses from agents/reviewer.md';
  for (const l of lenses) if (!hint.includes(l)) problems.push(`lens ${l} is not an argument`);
  for (const l of require('./config').DEFAULTS.review.lenses) {
    if (!skill.includes('`' + l + '`')) problems.push(`default lens ${l} is not named`);
  }
  if (!/do not move the flow/i.test(skill)) problems.push('the skill does not say it leaves the flow alone');
  return problems.length ? problems.join('; ') : null;
});

scenario('every agent file agrees with AGENT_DEFAULTS, so reset is not destructive', () => {
  const root = path.join(__dirname, '..');
  const cli = fs.readFileSync(path.join(root, 'lib', 'cli.js'), 'utf8');
  const grab = (name) => {
    const m = cli.match(new RegExp('const ' + name + ' = \\{[\\s\\S]*?\\};'));
    return m ? eval('(' + m[0].replace(new RegExp('^const ' + name + ' = '), '').replace(/;$/, '') + ')') : null;
  };
  const models = grab('AGENT_DEFAULTS');
  const efforts = grab('AGENT_EFFORTS');
  if (!models || !efforts) return 'could not read AGENT_DEFAULTS/AGENT_EFFORTS';
  const bad = [];
  for (const agent of Object.keys(models)) {
    const f = path.join(root, 'agents', `${agent}.md`);
    if (!fs.existsSync(f)) { bad.push(`${agent}: no agents/${agent}.md`); continue; }
    const txt = fs.readFileSync(f, 'utf8');
    const m = (txt.match(/^model:[ \t]*(.+)$/m) || [])[1];
    const e = (txt.match(/^effort:[ \t]*(.+)$/m) || [])[1];
    // Drift here is silent until somebody runs `keel models reset`, which then reverts a
    // deliberate choice to whatever the map says. It cost the opus/high RED and GREEN authors once.
    if (m !== models[agent]) bad.push(`${agent}: file says model ${m}, AGENT_DEFAULTS says ${models[agent]}`);
    if (e !== efforts[agent]) bad.push(`${agent}: file says effort ${e}, AGENT_EFFORTS says ${efforts[agent]}`);
  }
  return bad.length ? bad.join('; ') : null;
});

scenario('single mode commits test and code together; paired still refuses', (dir) => {
  startRed(dir);
  // Default is paired, and a green commit must still refuse a test file.
  keel(['state', 'phase', 'green'], dir);
  const src = path.join(dir, 'apps/api/src/main/kotlin/app/S.kt');
  const tst = path.join(dir, 'apps/api/src/test/kotlin/app/STest.kt');
  fs.mkdirSync(path.dirname(src), { recursive: true });
  fs.mkdirSync(path.dirname(tst), { recursive: true });
  fs.writeFileSync(src, 'package app\nfun s() = 1\n');
  fs.writeFileSync(tst, 'package app\nclass STest\n');
  const paired = keel(['commit', 'green', 'AC-001', 'both'], dir);
  if (paired.code === 0) return 'paired mode must refuse a green commit carrying a test file';
  // Now single mode: the same two files in one commit, under the feat( prefix trace needs.
  fs.appendFileSync(path.join(dir, '.keel/config.yml'), '\nloops:\n  commit_style: single\n');
  keel(['state', 'phase', 'gate'], dir);
  keel(['state', 'phase', 'ac'], dir);
  const single = keel(['commit', 'ac', 'AC-001', 'test and code'], dir);
  if (single.code !== 0) return `single mode should allow both: ${single.out.trim()}`;
  return /feat\(AC-001\)/.test(single.out) ? null : `must keep the feat( prefix: ${single.out.trim()}`;
});

scenario('default config is paired, so an upgrade changes nothing', (dir) => {
  const r = keel(['state', 'show'], dir);
  if (r.code !== 0) return `state show failed: ${r.out.trim()}`;
  const cfg = require('./config').load(dir);
  return (cfg.loops || {}).commit_style === 'paired'
    ? null : `default commit_style should be paired, got ${(cfg.loops || {}).commit_style}`;
});

scenario('a skipped ship step is recorded, and a deferred one says the push still stops', (dir) => {
  startRed(dir);
  const need = keel(['state', 'ship-skip', 'deps'], dir);
  if (need.code === 0) return 'a skip must require a reason';
  const deferred = keel(['state', 'ship-skip', 'coverage', '--reason', 'CI runs it'], dir);
  if (!/does not remove the gate/.test(deferred.out)) {
    return `skipping coverage must say the push gate still applies: ${deferred.out.trim()}`;
  }
  const optional = keel(['state', 'ship-skip', 'security', '--reason', 'docs only'], dir);
  if (/does not remove the gate/.test(optional.out)) {
    return 'security is genuinely optional and must not claim a push gate';
  }
  const listed = keel(['state', 'ship-skip'], dir).out;
  return /coverage: CI runs it/.test(listed) && /security: docs only/.test(listed)
    ? null : `both skips should be listed: ${listed.trim()}`;
});

scenario('coverage.enabled false drops the gate; gate_commands [] never did', (dir) => {
  const cfgPath = path.join(dir, '.keel', 'config.yml');
  const gates = require('./gates');
  const load = () => { delete require.cache[require.resolve('./config')]; return require('./config').load(dir); };
  const names = (c) => gates.pushBlockers(c).map((b) => b.gate);
  if (!names(load()).includes('coverage')) return 'coverage should gate a push by default';
  // The trap: an empty list reads as "unset" in matchesAnyCommand and falls back to the
  // defaults, so it looks like a way to disable the gate and silently is not.
  fs.appendFileSync(cfgPath, '\ncoverage:\n  gate_commands: []\n');
  if (!names(load()).includes('coverage')) return 'gate_commands: [] must NOT disable the gate';
  // The real switch.
  fs.appendFileSync(cfgPath, '  enabled: false\n');
  return names(load()).includes('coverage') ? 'coverage.enabled: false must drop the gate' : null;
});

/* --------------------------------------------------- the event journal */

function readEvents(dir) {
  const p = path.join(dir, '.keel', 'logs', 'events.jsonl');
  if (!fs.existsSync(p)) return [];
  return fs.readFileSync(p, 'utf8').split('\n').filter(Boolean).map((l) => JSON.parse(l));
}

scenario('always: a logging failure never blocks the tool call', (dir) => {
  startRed(dir);
  // Every way the log can fail: no payload, unparseable payload, and a logs path that cannot
  // be written. A non-zero exit from any of these would refuse a tool the guards allowed.
  const bad = [
    hook('log-tool', {}, dir),
    keel(['hook', 'log-tool'], dir, 'not json at all'),
    keel(['hook', 'log-fail'], dir, ''),
  ];
  for (const r of bad) if (r.code !== 0) return `a malformed log payload exited ${r.code}: ${r.out.trim()}`;
  // .keel/logs taken by a regular file, so mkdir and append both fail.
  fs.rmSync(path.join(dir, '.keel', 'logs'), { recursive: true, force: true });
  fs.writeFileSync(path.join(dir, '.keel', 'logs'), 'not a directory');
  const blocked = hook('log-tool', { tool_name: 'Edit', tool_input: { file_path: 'a.kt' } }, dir);
  fs.unlinkSync(path.join(dir, '.keel', 'logs'));
  return blocked.code === 0 ? null : `an unwritable log exited ${blocked.code}: ${blocked.out.trim()}`;
});

scenario('a tool call is journalled once, with its result', (dir) => {
  startRed(dir);
  hook('log-tool', { tool_name: 'Edit', tool_input: { file_path: path.join(dir, 'apps/api/src/test/kotlin/ATest.kt') }, tool_response: { duration_ms: 312 } }, dir);
  hook('log-fail', { tool_name: 'Bash', tool_input: { command: './gradlew test' }, tool_response: { stderr: 'expected: 422 but was: 200' } }, dir);
  const evs = readEvents(dir).filter((e) => e.kind === 'tool');
  if (evs.length !== 2) return `expected 2 tool events, got ${evs.length}`;
  const edit = evs.find((e) => e.tool === 'Edit');
  const bash = evs.find((e) => e.tool === 'Bash');
  if (!edit || edit.ok !== true) return 'the successful Edit was not recorded as ok';
  if (edit.arg !== 'ATest.kt') return `a path should log as its basename, got "${edit.arg}"`;
  if (edit.ms !== 312) return 'a duration the payload carried should be kept';
  if (!bash || bash.ok !== false) return 'the failing Bash was not recorded as a failure';
  if (!/422/.test(bash.detail || '')) return 'the failure detail was dropped';
  if (bash.ac !== 'AC-001' || bash.phase !== 'red') return 'a tool event must carry the AC and phase it happened in';
  return null;
});

scenario('a duration keel was not given is left out rather than invented', (dir) => {
  startRed(dir);
  hook('log-tool', { tool_name: 'Read', tool_input: { file_path: 'a.kt' } }, dir);
  const e = readEvents(dir).find((x) => x.kind === 'tool');
  return e && e.ms === undefined ? null : `expected no ms field, got ${e && e.ms}`;
});

scenario('a project with no flow running is never journalled', (dir) => {
  // No `keel state start` — the repo is configured but idle.
  hook('log-tool', { tool_name: 'Edit', tool_input: { file_path: 'a.kt' } }, dir);
  return readEvents(dir).length ? 'an idle project must not accumulate a log' : null;
});

scenario('phase transitions and gate decisions journal themselves', (dir) => {
  startRed(dir);
  keel(['state', 'phase', 'green'], dir);
  const st = require('./state');
  st.update({ root: dir }, (s) => { s.gates.log.push('ac AC-001 approved'); });
  const evs = readEvents(dir);
  const phase = evs.find((e) => e.kind === 'phase' && e.from === 'red' && e.to === 'green');
  if (!phase) return 'the red -> green transition was not recorded';
  const gate = evs.find((e) => e.kind === 'gate');
  if (!gate || gate.ac !== 'AC-001' || gate.verdict !== 'approved') return `the gate decision was not parsed: ${JSON.stringify(gate)}`;
  // A write that changes neither phase nor gates must not manufacture an event.
  const before = evs.length;
  st.update({ root: dir }, (s) => { s.last_failure = 'noise'; });
  const after = readEvents(dir).length;
  if (after !== before) return `a state write that changed neither added ${after - before} event(s)`;
  return null;
});

scenario('a guard denial is recorded, not only printed', (dir) => {
  startRed(dir);
  const r = hook('pre-tool', { tool_name: 'Edit', tool_input: { file_path: path.join(dir, 'apps/api/src/main/kotlin/A.kt') } }, dir);
  if (r.code !== 2) return 'production code must still be denied in RED';
  const g = readEvents(dir).find((e) => e.kind === 'guard');
  if (!g) return 'the denial left no trace';
  if (g.tool !== 'Edit' || !g.denied) return `the guard event is incomplete: ${JSON.stringify(g)}`;
  return null;
});

scenario('an agent verdict is captured instead of being thrown away', (dir) => {
  startRed(dir);
  hook('subagent-start', { agent_type: 'keel:test-author' }, dir);
  hook('subagent-stop', { agent_type: 'keel:test-author', last_message: 'done.\nRED-RESULT: failing' }, dir);
  const evs = readEvents(dir).filter((e) => e.kind === 'agent');
  const start = evs.find((e) => e.ev === 'start');
  const stop = evs.find((e) => e.ev === 'stop');
  if (!start) return 'the agent start was not recorded';
  if (!stop || !/RED-RESULT:\s*failing/i.test(stop.verdict || '')) return `the verdict was not captured: ${JSON.stringify(stop)}`;
  return null;
});

scenario('the journal rotates rather than growing without bound', (dir) => {
  startRed(dir);
  const events = require('./events');
  const cfg = { root: dir };
  fs.mkdirSync(events.dir(cfg), { recursive: true });
  fs.writeFileSync(events.file(cfg), 'x'.repeat(events.CAP_BYTES + 10));
  events.append(cfg, { kind: 'phase', from: 'a', to: 'b' });
  if (!fs.existsSync(path.join(events.dir(cfg), 'events.1.jsonl'))) return 'the oversized log was not rotated aside';
  const now = fs.readFileSync(events.file(cfg), 'utf8');
  return now.length < 500 ? null : 'the live log should start fresh after a rotation';
});

scenario('the MCP server answers a handshake and lists its tools', () => {
  const r = spawnSync('node', [path.join(__dirname, '..', 'mcp', 'server.js')], {
    encoding: 'utf8',
    input: [
  ...
  if (!init || !init.result || !init.result.serverInfo) return `no initialize result: ${r.stdout || r.stderr}`;
  const list = lines.find((l) => l.id === 2);
  const names = ((list && list.result && list.result.tools) || []).map((t) => t.name);
  for (const want of ['keel_status', 'keel_projects', 'keel_timeline', 'keel_next', 'keel_explain', 'keel_dashboard']) {
    if (!names.includes(want)) return `tools/list is missing ${want}: ${names.join(', ')}`;
  }
  // A notification carries no id and must draw no response.
  if (lines.some((l) => l.id === undefined || l.id === null)) return 'the server replied to a notification';
  return null;
});

scenario('the dashboard view agrees with the board on phase, AC and blockers', (dir) => {
  startRed(dir);
  const view = require('../mcp/view').build(dir);
  const state = require('./state').read({ root: dir });
  if (!view.active) return 'a started flow must read as active';
  if (view.flow.phase !== state.phase) return `view says phase ${view.flow.phase}, state says ${state.phase}`;
  if (view.acs.current !== state.current) return 'the view and the state disagree on the current AC';
  const cfg = require('./config').load(dir);
  const fromBoard = require('./gates').pushBlockers(cfg).map((b) => b.gate).sort();
  const fromView = view.blockers.map((b) => b.gate).sort();
  if (fromBoard.join() !== fromView.join()) return `blockers differ: board ${fromBoard.join()} vs view ${fromView.join()}`;
  if (view.next !== require('./hooks').nextStep(state)) return 'the view and the session brief disagree on the next step';
  if (!require('./board').RAILS[view.flow.flow]) return 'the view named a flow with no rail';
  return null;
});

// The machine's project list, pointed at a home of its own so a scenario sees only what it made.
function withHome(fn) {
  const before = process.env.KEEL_HOME;
  process.env.KEEL_HOME = fs.mkdtempSync(path.join(os.tmpdir(), 'keel-home-'));
  try { return fn(process.env.KEEL_HOME); } finally {
    try { fs.rmSync(process.env.KEEL_HOME, { recursive: true, force: true }); } catch (e) { /* gone */ }
    process.env.KEEL_HOME = before;
  }
}

scenario('projects: a session start puts the project on the list, and a dead one drops off', (dir) => withHome(() => {
  const other = fakeRepo();
  try {
    hook('session-start', { source: 'startup' }, dir);
    hook('session-start', { source: 'startup' }, other);
    const projects = require('./projects');
    const roots = projects.list().map((p) => p.root);
    for (const d of [dir, other]) if (!roots.includes(path.resolve(d))) return `${d} is not on the list: ${roots.join(', ')}`;
    if (!projects.resolve(path.basename(other))) return 'a project cannot be found by its name';
    fs.rmSync(path.join(other, '.keel', 'config.yml'));
    if (projects.list().some((p) => p.root === path.resolve(other))) return 'a project without keel is still listed';
    const r = keel(['projects', 'forget', path.basename(dir)], dir);
    if (r.code !== 0) return `forget failed: ${r.out.trim()}`;
    return projects.list().length ? `forget left ${projects.list().map((p) => p.name).join(', ')}` : null;
  } finally { fs.rmSync(other, { recursive: true, force: true }); }
}));

scenario('projects: eight sessions registering at once lose no entry', () => withHome((home) => {
  const base = fs.mkdtempSync(path.join(os.tmpdir(), 'keel-many-'));
  try {
    const lib = JSON.stringify(path.join(__dirname, 'projects'));
    const cmds = [];
    for (let i = 0; i < 8; i++) {
      const d = path.join(base, 'p' + i);
      fs.mkdirSync(path.join(d, '.keel'), { recursive: true });
      fs.writeFileSync(path.join(d, '.keel', 'config.yml'), 'version: 2\n');
      cmds.push(`node -e 'require(${lib}).register(${JSON.stringify(d)})' &`);
    }
    const r = sh(`${cmds.join('\n')}\nwait`, base);
    if (r.code !== 0) return `a registration failed: ${r.out}`;
    const n = require('./projects').list().length;
    if (n !== 8) return `expected 8 projects on the list, found ${n}`;
    return fs.readdirSync(home).some((f) => /\.(lock|tmp)$/.test(f)) ? 'a lock or temp file was left behind' : null;
  } finally { fs.rmSync(base, { recursive: true, force: true }); }
}));

scenario('dashboard: one hub serves every project, a second session joins it, and a survivor takes over', (dir) => withHome(() => {
  const other = fakeRepo();
  const port = 20000 + Math.floor(Math.random() * 20000);
  const hubJs = JSON.stringify(path.join(__dirname, '..', 'mcp', 'http'));
  // The first session's hub, in a process of its own so it can die like a closed session does.
  const first = `const h=require(${hubJs});h.start(${JSON.stringify(dir)}).then((r)=>{process.stdout.write(JSON.stringify(r)+'\\n');setInterval(()=>{},1000);});`;
  const script = `
    const { spawn } = require('child_process');
    const http = require('http');
    const get = (p, host) => new Promise((ok) => {
      const req = http.get({ host: '127.0.0.1', port: ${port}, path: p, headers: host ? { host } : {} }, (res) => {
        let b = ''; res.on('data', (d) => { b += d; }); res.on('end', () => ok({ code: res.statusCode, body: b }));
      });
      req.on('error', () => ok({ code: 0, body: '' }));
    });
    const wait = (ms) => new Promise((ok) => setTimeout(ok, ms));
    const until = async (fn, ms) => { const end = Date.now() + ms; while (Date.now() < end) { if (await fn()) return true; await wait(100); } return false; };
    (async () => {
      const a = spawn(process.execPath, ['-e', ${JSON.stringify(first)}], { stdio: ['ignore', 'pipe', 'inherit'] });
      const started = await new Promise((ok) => a.stdout.once('data', (d) => ok(JSON.parse(String(d)))));
      if (!started.hub) throw new Error('the first session did not become the hub');
      const hub = require(${hubJs});
      const b = await hub.start(${JSON.stringify(other)});
      if (b.hub || b.port !== ${port}) throw new Error('the second session opened its own dashboard: ' + JSON.stringify(b));
      const both = await until(async () => {
        const r = await get('/api/projects');
        return r.code === 200 && JSON.parse(r.body).projects.length === 2;
      }, 5000);
      if (!both) throw new Error('the hub never listed both projects: ' + (await get('/api/projects')).body);
      const list = JSON.parse((await get('/api/projects')).body).projects;
      const mine = list.find((p) => p.id === b.id);
      if (!mine) throw new Error('the second session\\'s project is not the one on the list');
      const v = await get('/api/view?project=' + mine.id);
      if (v.code !== 200 || JSON.parse(v.body).root !== ${JSON.stringify(path.resolve(other))}) throw new Error('the view for the second project is wrong: ' + v.body.slice(0, 200));
      if ((await get('/api/view?project=nope')).code !== 404) throw new Error('an unknown project did not 404');
      if ((await get('/api/projects', 'evil.example:${port}')).code !== 403) throw new Error('a foreign Host header was served');
      a.kill();
      const took = await until(async () => {
        const r = await get('/api/hello');
        return r.code === 200 && JSON.parse(r.body).pid === process.pid;
      }, 8000);
      if (!took) throw new Error('nobody took over the port after the hub went away');
      const after = await until(async () => {
        const r = await get('/api/projects');
        return r.code === 200 && JSON.parse(r.body).projects.length === 2;
      }, 5000);
      if (!after) throw new Error('the new hub lost a project');
      hub.stop();
  ...
  try {
    const r = spawnSync(process.execPath, ['-e', script], {
      ...
      env: Object.assign({}, process.env, { KEEL_DASHBOARD_PORT: String(port), KEEL_TAKEOVER_MS: '300' }),
    });
    return /PASS/.test(r.stdout || '') ? null : (r.stdout || '') + (r.stderr || '') || 'the hub scenario timed out';
  } finally { fs.rmSync(other, { recursive: true, force: true }); }
}));

// The fixture config is current, so age it back to exercise a real upgrade.
function ageConfig(dir) {
  const f = path.join(dir, '.keel/config.yml');
  fs.writeFileSync(f, fs.readFileSync(f, 'utf8').replace(/^version: \d+$/m, 'version: 1'));
  return f;
}

scenario('upgrade: the dry run lists what it would add and writes nothing', (dir) => {
  const f = ageConfig(dir);
  const before = fs.readFileSync(f, 'utf8');
  const r = keel(['upgrade'], dir);
  if (expectOk(r, 'red_author')) return expectOk(r, 'red_author');
  if (!/version 1 -> 2/.test(r.out)) return `no version bump reported: ${r.out.trim()}`;
  if (!/upgrade --write/.test(r.out)) return 'no --write hint';
  return fs.readFileSync(f, 'utf8') === before ? null : 'the dry run modified config.yml';
});

scenario('upgrade --write keeps comments and user values while adding keys', (dir) => {
  const f = ageConfig(dir);
  fs.appendFileSync(f, '\n# my own note, keep me\nloops:\n  stall_repeats: 9\n');
  const r = keel(['upgrade', '--write'], dir);
  if (r.code !== 0) return `upgrade failed: ${r.out.trim()}`;
  const text = fs.readFileSync(f, 'utf8');
  const checks = [
    [/# my own note, keep me/, 'the user comment was lost'],
    [/stall_repeats: 9/, 'a user value was overwritten'],
    [/^ {2}red_author: main/m, 'red_author was not added with the right indent'],
    [/# main \| subagent/, 'the template comment did not come along'],
    [/^version: 2$/m, 'the version was not bumped'],
  ];
  const bad = checks.find(([re]) => !re.test(text));
  if (bad) return bad[1] + ':\n' + text;
  if (/red_author/.test(text.split('loops:')[0])) return 'red_author landed outside the loops block';
  return expectOk(keel(['status'], dir));                 // and it still parses
});

scenario('upgrade adds missing state fields without touching the flow', (dir) => {
  ageConfig(dir);
  fs.writeFileSync(path.join(dir, '.keel/state.json'),
    JSON.stringify({ flow: 'feature', phase: 'red', acs: {}, gates: { mode: 'every-ac', skipped: {}, log: [] } }, null, 2));
  const r = keel(['upgrade', '--write'], dir);
  if (r.code !== 0) return `upgrade failed: ${r.out.trim()}`;
  const s = JSON.parse(fs.readFileSync(path.join(dir, '.keel/state.json'), 'utf8'));
  if (!s.lanes || typeof s.lanes !== 'object') return 'state.lanes was not added';
  return s.flow === 'feature' && s.phase === 'red' ? null : 'the migration clobbered existing state';
});

scenario('upgrade is idempotent', (dir) => {
  const f = ageConfig(dir);
  if (keel(['upgrade', '--write'], dir).code !== 0) return 'the first upgrade failed';
  const once = fs.readFileSync(f, 'utf8');
  const again = keel(['upgrade', '--write'], dir);
  if (expectOk(again, 'already up to date')) return expectOk(again, 'already up to date');
  return fs.readFileSync(f, 'utf8') === once ? null : 'the second run changed the file';
});

scenario('upgrade refuses a missing config and one newer than the plugin', (dir) => {
  const f = path.join(dir, '.keel/config.yml');
  fs.writeFileSync(f, fs.readFileSync(f, 'utf8').replace(/^version: \d+$/m, 'version: 99'));
  const newer = keel(['upgrade', '--write'], dir);
  if (expectFails(newer, 'newer keel')) return expectFails(newer, 'newer keel');
  if (fs.readFileSync(f, 'utf8').includes('red_author')) return 'it wrote to a newer config anyway';

  const fresh = fs.mkdtempSync(path.join(os.tmpdir(), 'keel-noconf-'));
  sh('git init -q && git config user.email s@x && git config user.name s', fresh);
  const r = keel(['upgrade'], fresh);
  fs.rmSync(fresh, { recursive: true, force: true });
  return expectFails(r, 'keel init --write');
});

scenario('a stale config is announced at session start', (dir) => {
  ageConfig(dir);
  const r = hook('session-start', { source: 'startup' }, dir);
  return /keel upgrade/.test(r.out) ? null : `no upgrade nudge: ${r.out.trim()}`;
});

scenario('every flow graphs as a connected, non-overlapping figure', (dir) => {
  const st = require('./state');
  const board = require('./board');
  const view = require('../mcp/view');
  const problems = [];
  const start = { feature: 'red', change: 'green', fix: 'bug-investigate', hunt: 'hunt-prove' };
  for (const [flow, phase] of Object.entries(start)) {
    st.write({ root: dir }, Object.assign({}, st.EMPTY, { flow, phase, gates: { mode: 'every-ac', log: [] } }));
    const g = view.build(dir).graph;
    if (!g) { problems.push(`${flow}: no graph`); continue; }
    const names = new Set(g.nodes.map((n) => n.phase));
    // The rail is the spine and must be drawn whole.
    for (const p of board.RAILS[flow]) if (!names.has(p)) problems.push(`${flow}: rail phase ${p} missing`);
    // A phase on another flow's rail is a handoff, not a detour: following those once rendered
    // the entire feature pipeline inside the change flow's graph.
    for (const [other, rail] of Object.entries(board.RAILS)) {
      if (other === flow) continue;
      for (const p of rail) {
        if (board.RAILS[flow].includes(p)) continue;
        if (names.has(p)) problems.push(`${flow}: drew ${p}, which belongs to the ${other} rail`);
      }
    }
    if (g.nodes.filter((n) => n.current).length !== 1) problems.push(`${flow}: not exactly one current node`);
    const seen = new Set();
    for (const n of g.nodes) {
      const cell = `${n.row}:${n.col}`;
      if (seen.has(cell)) problems.push(`${flow}: two nodes at ${cell}`);
      seen.add(cell);
      if (!Number.isFinite(n.x) || !Number.isFinite(n.y)) problems.push(`${flow}: ${n.phase} has no position`);
    }
    for (const e of g.edges) {
      if (!names.has(e.from) || !names.has(e.to)) problems.push(`${flow}: edge ${e.from}->${e.to} leaves the figure`);
      if (/NaN|undefined/.test(e.d)) problems.push(`${flow}: unusable path for ${e.from}->${e.to}`);
    }
    // Every legal move out of the current phase that is drawn must be reachable by an edge.
    for (const t of st.TRANSITIONS[phase] || []) {
      if (!names.has(t)) continue;
      if (!g.edges.some((e) => e.from === phase && e.to === t && e.live)) {
        problems.push(`${flow}: ${phase} -> ${t} is legal but not drawn live`);
      }
    }
  }
  return problems.length ? problems.join('; ') : null;
