...
const RAILS = {
  ...
  // The hunt ends at triage rather than at ship: it has nothing to ship. What comes out of it
  // is a backlog, and each item leaves through a flow of its own.
  hunt: ['hunt-scope', 'hunt-sweep', 'hunt-prove', 'hunt-report', 'hunt-triage'],
};
const SHORT = {
...
function render(cfg, state, opts = {}) {
  ...
  if (!st.active(state)) {
    return [`no active flow. Config: ${cfg.configured ? '.keel/config.yml' : 'missing — run `keel init --write`'}`,
      'Start one with /keel:feature, /keel:change, /keel:fix or /keel:hunt.'].join('\n');
  }
...
function fingerprint(cfg) {
  const parts = [];
  // hunt.json carries a `rev` that every backlog mutation bumps, so fingerprinting the pointer
  // is enough for `--watch` to reprint as verdicts land.
  for (const f of ['state.json', 'coverage.json', 'security.json', 'hunt.json']) {
...
module.exports = { render, fingerprint, rail, acGrid, agentRows, RAILS, SHORT };
