...
// The phases a --semi hunt stops at. Each is approved once, and the command that consumes that
// phase's output refuses until it is — the same shape as `hunt add` refusing an unconfirmed lens
// set, which is the one gate in keel that demonstrably holds.
const GATES = ['sweep', 'prove', 'report'];
...
function start(cfg, opts = {}) {
  // The first question of the flow, and it is asked by refusing. Deliberately *not* a
  // `keel ask` blocking question: those also block `ladder`, `init --write`, `memory update` and
  // `preflight`, and how closely you want to supervise a hunt has no business stopping unrelated
  // work. Refusing to start the hunt is the whole gate — nothing proceeds because nothing exists.
  const autonomy = opts.auto ? 'auto' : (opts.semi ? 'semi' : null);
  if (opts.auto && opts.semi) return { ok: false, out: 'pick one: --auto or --semi.', usage: true };
  if (!autonomy) {
    return { ok: false, out: ['how closely do you want to watch this hunt? It is the one thing to decide up front.',
      '',
      '  keel hunt start --semi    a gate at each phase: after the sweep, before spending provers,',
      '                            and after grouping. You see what was proposed before it is proved.',
      '  keel hunt start --auto    runs through. The lens set is confirmed by the model and marked as',
      '                            such, and the report says which gates no human saw.',
      '',
      'Either way nothing is fixed inside a hunt: the output is a report and a backlog, and each',
      'finding is drained into /keel:fix separately. --auto is safe to let run.'].join('\n') };
  }
  ...
  // A fast run narrows the lens set and the scope, and says so on everything it produces. It does
  // not touch the proof bar: the report still refuses to render while any candidate is unverified.
  const fast = !!opts.fast;
  const scope = opts.scope || (fast ? 'diff' : cfg.hunt.default_scope);
  ...
  let proposed = fast ? (cfg.hunt.fast_lenses || []).filter((l) => configured.includes(l)) : configured;
  if (fast && !proposed.length) return { ok: false, out: 'hunt.fast_lenses names no configured lens.' };
  if (opts.lenses) {
  ...
  const run_ = {
    ...
    lenses: { configured, proposed, confirmed: null, confirmed_at: null, confirmed_by: null, swept: {} },
    stack: {
    ...
    fast,
    autonomy,
    // In --auto every gate is pre-approved by the model, so the record has the same shape either
    // way and the report can say which gates a human actually saw.
    gates: GATES.reduce((acc, g) => Object.assign(acc, {
      [g]: autonomy === 'auto' ? { decision: 'approve', by: 'model', note: 'started with --auto', at: new Date().toISOString() } : null,
    }), {}),
    rev: 0,
    next_id: 1,
    next_group: 1,
  ...
  const lines = [`hunt ${id} started (--${autonomy}${fast ? ', --fast' : ''}) — scope ${run_.scope.mode}${run_.scope.paths.length ? ` (${run_.scope.paths.join(', ')})` : ''}, ${String(sha).slice(0, 7)} on ${branch}`,
  ...
  if (fast) {
    const off = configured.filter((l) => !proposed.includes(l));
    lines.push(`  — a fast run, so these are not looked at: ${off.join(', ')}`);
    lines.push('  Every finding is still proved before it is reported; --fast narrows what is looked at, not that.');
  }
  if (autonomy === 'semi') {
    lines.push('', `gates: ${GATES.join(', ')} — each one waits for \`keel hunt gate <phase> approve\``);
  }
...
function lenses(cfg, opts = {}) {
  ...
  run_.lenses.confirmed_by = String(opts.by && opts.by !== true ? opts.by : (run_.autonomy === 'auto' ? 'model' : 'user')).toLowerCase();
  writeRun(cfg, run_);
  const pairs = sweepPairs(cfg, list);
  return { ok: true, out: [`confirmed ${list.length} lens(es): ${list.join(', ')}`
    + (run_.lenses.confirmed_by === 'model' ? ' — by the model itself, recorded as a self-answer' : ''),
    '',
...
function add(cfg, opts = {}) {
  ...
  const cap = run_.fast ? Math.max(1, Math.ceil(cfg.hunt.max_candidates_per_lens / 2)) : cfg.hunt.max_candidates_per_lens;
  if (list.length > cap) {
...
function prove(cfg, id, opts = {}) {
  ...
  const shut = gateBlocked(run_, 'sweep', 'recording a verdict');
  if (shut) return { ok: false, out: shut };
...
function next(cfg, opts = {}) {
  const run_ = readRun(cfg);
  if (!run_) return { ok: false, out: 'no hunt open. Start one with `keel hunt start`.' };
  const shut = gateBlocked(run_, 'report', 'handing a finding over');
  if (shut) return { ok: false, out: shut };
  const c = counts(run_);
...
// Approve or reject one phase's gate. Records who decided, because `--by model` is a self-answer and
// has to be visible as one.
function gate(cfg, which, decision, opts = {}) {
  const run_ = readRun(cfg);
  if (!run_) return { ok: false, out: 'no hunt open. Start one with `keel hunt start --semi` or `--auto`.' };
  if (!GATES.includes(String(which))) {
    return { ok: false, out: `usage: keel hunt gate <${GATES.join('|')}> approve|reject [--note "…"] [--by user]`, usage: true };
  }
  const d = String(decision || '').toLowerCase();
  if (!['approve', 'reject'].includes(d)) {
    return { ok: false, out: `usage: keel hunt gate ${which} approve|reject [--note "…"] [--by user]`, usage: true };
  }
  const by = String(opts.by && opts.by !== true ? opts.by : 'model').toLowerCase();
  run_.gates = run_.gates || {};
  run_.gates[which] = { decision: d, by, note: opts.note && opts.note !== true ? String(opts.note) : null,
    at: new Date().toISOString() };
  writeRun(cfg, run_);
  const left = GATES.filter((g) => !(run_.gates[g] && run_.gates[g].decision === 'approve'));
  return { ok: true, out: [`gate ${which}: ${d}${by === 'model' ? ' by the model itself — recorded as a self-answer' : ` by the ${by}`}`,
    left.length ? `still waiting on: ${left.join(', ')}` : 'every gate is approved.'].join('\n') };
}

// The refusal a gated command prints, or null when the gate is open.
function gateBlocked(run_, which, what) {
  const g = (run_.gates || {})[which];
  if (g && g.decision === 'approve') return null;
  if (g && g.decision === 'reject') {
    return `the ${which} gate was rejected${g.note ? `: ${g.note}` : ''}. Re-approve it with \`keel hunt gate ${which} approve\` once the reason is addressed.`;
  }
  return [`${what} is behind the ${which} gate, and this hunt was started with --semi.`,
    `  keel hunt gate ${which} approve --by user`,
    'A hunt started with --auto pre-approves every gate and stops at none of them.'].join('\n');
}
...
function candidates(cfg, opts = {}) {
  const run_ = readRun(cfg);
  if (!run_) return { ok: false, out: 'no hunt open. Start one with `keel hunt start`.' };
  const shut = gateBlocked(run_, 'sweep', 'handing out candidates to prove');
  if (shut) return { ok: false, out: shut };
...
// A fast run's report must be legible as one: short because it looked at less, not because there was
// less to find. Both pages name the lenses that never ran.
// What a reader needs to know about how much of this was supervised.
function trustBanner(run_) {
  const L = [];
  if (run_.lenses && run_.lenses.confirmed_by === 'model') {
    L.push('', '> **The lens set was confirmed by the model, not reviewed.** Nobody chose what this hunt',
      '> would and would not look through.');
  }
  const unseen = GATES.filter((g) => {
    const x = (run_.gates || {})[g];
    return x && x.decision === 'approve' && x.by === 'model';
  });
  if (unseen.length) {
    L.push('', `> **${unseen.length} of ${GATES.length} gates were approved by the model** (${unseen.join(', ')}),`,
      '> because this hunt was started with `--auto`. Everything reported was still proved; what no one',
      '> checked is whether the right things were looked for.');
  }
  return L;
}

function fastBanner(cfg, run_) {
  if (!run_.fast) return [];
  const off = (cfg.hunt.lenses || []).filter((l) => !(run_.lenses.confirmed || run_.lenses.proposed || []).includes(l));
  return ['', '> **This was a `--fast` hunt.** It looked through '
    + `${(run_.lenses.confirmed || []).join(', ')} only`
    + (off.length ? `, and never looked through ${off.join(', ')}.` : '.'),
    '> Everything reported here was still proved against the running stack — `--fast` narrows what is',
    '> examined, never whether a finding is verified. A short report is not a clean bill of health.'];
}
...
function render(cfg, run_) {
  ...
  L.push(...fastBanner(cfg, run_));
  L.push(...trustBanner(run_));
  L.push('', '| status | count |', '|---|---|');
...
function report(cfg, opts = {}) {
  ...
  // The real report is behind the prove gate. `--candidates` never is: it is the page you read in
  // order to decide the sweep gate, so gating it would be a deadlock.
  const shut = gateBlocked(run_, 'prove', 'rendering the report');
  if (shut) return { ok: false, out: shut };
...
function resume(cfg) {
  ...
  const waiting = GATES.find((g) => !((run_.gates || {})[g] || {}).decision);
  L.push(`gates    ${GATES.map((g) => `${g}:${(((run_.gates || {})[g]) || {}).decision || 'waiting'}`).join('  ')}`);
  if (waiting) {
    L.push('', `next:`, `  keel hunt gate ${waiting} approve --by user      # this hunt is --semi and stops here`);
    return { ok: true, out: L.join('\n') };
  }
  L.push('', 'next:');
  if (left.length) {
...
function status(cfg) {
  ...
  lines.push('', `autonomy: --${run_.autonomy || 'auto'}`);
  lines.push(`gates: ${GATES.map((g) => { const x = (run_.gates || {})[g] || {}; return `${g}:${x.decision || 'waiting'}${x.by === 'model' ? '(model)' : ''}`; }).join('  ')}`);
...
module.exports = { start, lenses, add, prove, group, next, close, report, render, renderCandidates, list, status, candidates, resume, gate, gateBlocked, e2eCoverage, GATES,
