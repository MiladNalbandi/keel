  ...
  if (verdict === 'proven') {
    ...
    if (!opts.repro || opts.repro === true) {
      return { ok: false, out: `a proven verdict needs --repro <file>: the command that makes ${f.id} fail on demand.\n`
        + `Store it under .keel/hunt/repro/ as ${f.id}${RECIPE_EXTS.join(` or ${f.id}`)}.\n`
    ...
    const inside = path.relative(reproDir(cfg), abs);
    if (inside.startsWith('..') || path.isAbsolute(inside)) {
      return { ok: false, out: `the recipe must live under .keel/hunt/repro/ so it travels with the backlog; got ${rel}.` };
    }
    ...
    f.severity = severity;
    f.repro = path.join('.keel', 'hunt', 'repro', path.basename(abs));
...
// `role` is set for a group's lead, where the heading names the cause rather than the finding,
// so the id and severity still have to be stated. An ungrouped finding has both in its own
// heading already, and repeating them reads like two findings.
function findingBlock(run_, f, role) {
  const lines = [role ? `**${f.id}** · ${role} · ${f.lens} · ${sev(f)} · ${where(f)}`
    : `${f.lens} · ${where(f)}`, ''];
  lines.push(`- Symptom: ${f.symptom}`);
  if (f.repro) lines.push(`- Proved at \`${String(f.proved_sha).slice(0, 7)}\`: \`${f.repro}\``);
  if (f.evidence) lines.push(`- Evidence: ${f.evidence}`);
  if (f.close) lines.push(`- Closed as ${f.close.as}: ${f.close.note}`);
  return lines;
}
...
function render(cfg, run_) {
  ...
  if (grouped.length || ungrouped.length) {
    ...
    for (const g of grouped) {
      ...
      if (symptoms.length) {
        ...
        for (const s of symptoms) L.push(`| ${s.id} | ${s.lens} | ${sev(s)} | ${where(s)} | ${s.repro ? `\`${s.repro}\`` : '—'} |`);
      }
    }
    for (const f of ungrouped) {
      L.push('', `### ${f.id} · ${sev(f)} · ${f.title}`, '');
  ...
  if (suspected.length) {
    ...
    for (const f of suspected) {
      L.push('', `**${f.id}** · ${f.lens} · ${where(f)}`, '');
      L.push(`- Symptom as proposed: ${f.symptom}`);
      // Labelled as the lens agent's theory every time it is printed. It is the one field that
      // must never be mistaken for a finding, and never reaches keel:reproducer.
      if (f.claim) L.push(`- Claim (the lens agent's theory, not a verdict): ${f.claim}`);
      if (f.evidence) L.push(`- Verifier: unproven — ${f.evidence}`);
    }
  }
  ...
  L.push('', '---', '');
  L.push(`Generated by \`keel hunt report\` from \`${path.join('.keel', 'hunt', `${run_.id}.json`)}\`.`);
...
function report(cfg, opts = {}) {
  ...
  const rel = opts.out && opts.out !== true ? String(opts.out)
    : path.join(cfg.hunt.report_dir, `${run_.id}.md`);
  const abs = path.join(cfg.root, rel);
  fs.mkdirSync(path.dirname(abs), { recursive: true });
  fs.writeFileSync(abs, render(cfg, run_));
  run_.report = rel;
  writeRun(cfg, run_);
  const c = counts(run_);
  return { ok: true, out: [`wrote ${rel}`,
...
module.exports = { start, lenses, add, prove, group, next, close, report, render, list, status, candidates,
