...
function greenDone(cfg) {
  ...
  out([`${ac}: green. ${modReport}`.trim() + covLine + flaky, '',
    `Now commit the code: keel commit green ${ac} "<what it does>"`].join('\n'));
}
...
const COMMIT_RULES = {
  ...
  coverage: { prefix: () => 'test(coverage)', allow: ['api-test', 'web-test'], deny: ['api-main', 'web-src'] },
...
function status() {
  ...
  const board = sum.ids.map((id) => {
    const a = state.acs[id];
    const mark = a.status === 'done' ? 'done ' : a.status === 'green' ? 'green' : a.status === 'red' ? 'red  ' : 'todo ';
    return `  ${id} [${a.layer}] ${mark}${id === state.current ? ' <- current' : ''}`;
  });
  out([`${state.flow} (${state.size}) phase ${state.phase}   lane ${state.lane}   gates ${state.gates.mode}`,
    state.spec ? `spec ${state.spec}` : null,
    ...board,
    `${sum.done}/${sum.total} ACs done`,
    state.flaky.length ? `flaky tests recorded: ${state.flaky.length}` : null,
    state.stall.count ? `repeated failure count: ${state.stall.count}` : null,
...
function verifyCmd(args) {
  ...
  if (tier === 'contract') {
    const steps = [['contract lint', verify.cmdFor(cfg, 'contract_lint'), '.'],
      ['codegen', (cfg.commands || {}).codegen, '.'],
      ['api compile', verify.cmdFor(cfg, 'api_compile'), cfg.backend.dir],
      ['web typecheck', verify.cmdFor(cfg, 'web_typecheck'), cfg.frontend.dir]].filter((s) => s[1]);
    const r = verify.runSteps(cfg, steps);
    return r.ok ? out('contract checks pass.') : fail(r.report, 3);
  }
  if (tier === 'full') {
    const steps = [['api module suite', verify.cmdFor(cfg, 'api_test_module'), cfg.backend.dir],
      ['web module suite', verify.cmdFor(cfg, 'web_test_module'), cfg.frontend.dir],
      ['static checks', (cfg.commands || {}).static_checks, '.']].filter((s) => s[1]);
    const r = verify.runSteps(cfg, steps);
    return r.ok ? out('full suite passes.') : fail(r.report, 3);
  }
  if (tier === 'e2e') {
  ...
  if (tier === 'release') {
    const full = verify.runSteps(cfg, [['api module suite', verify.cmdFor(cfg, 'api_test_module'), cfg.backend.dir],
      ['web module suite', verify.cmdFor(cfg, 'web_test_module'), cfg.frontend.dir]].filter((s) => s[1]));
    if (!full.ok) return fail(full.report, 3);
    const e2eCmd = (cfg.commands || {}).e2e;
    if (e2eCmd) {
      const r = verify.runSteps(cfg, [['e2e', e2eCmd.replace(/\s*--grep\s*\{AC\}/, ''), '.']]);
  ...
  if (tier === 'coverage') {
    ...
    const cmds = [(cfg.commands || {}).coverage_api, (cfg.commands || {}).coverage_web].filter(Boolean);
    for (const cmd of cmds) {
      const r = run(cmd, { cwd: cfg.root, timeout: 900000 });
      if (r.code !== 0) return fail('coverage run failed:\n' + trim(r.out, 15), 3);
    }
