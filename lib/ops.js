...
const { run, git, trim, readJson, matchGlob } = require('./util');
...
function stack(cfg, args) {
  ...
  const env = { COMPOSE_PROJECT_NAME: project(cfg, lane), KEEL_PORT_OFFSET: String(portOffset(cfg, lane)) };
...
module.exports = { stack, lane, smoke, pr, prBody, checkSize, triage, migrationDrift, markMigrationsApplied, project, portOffset };
