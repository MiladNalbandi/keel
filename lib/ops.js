...
function lane(cfg, args) {
  ...
  if (sub === 'start') {
    ...
    const parent = st.read(cfg);
    if (!st.active(parent)) return { ok: false, out: 'no active flow. Start one before splitting a lane.' };
    const given = args.includes('--acs') ? String(args[args.indexOf('--acs') + 1] || '').split(',').map((x) => x.trim()).filter(Boolean) : [];
    const unknown = given.filter((id) => !parent.acs[id]);
    if (unknown.length) return { ok: false, out: `no such acceptance criteria: ${unknown.join(', ')}` };
    const mine = given.length ? given : st.acList(parent).filter((id) => st.acLane(parent.acs[id]) === name);
    if (!mine.length) return { ok: false, out: `no ${name} acceptance criteria to hand over. Register them first, or name them with --acs AC-004,AC-005.` };

    const r = git(`worktree add -b ${branch} ${JSON.stringify(dir)}`, cfg.root);
    if (r.code !== 0) return { ok: false, out: trim(r.out, 10) };

    // The lane commits on its own branch. If its state file is not ignored it rides along, and
    // the merge back then collides with the parent's own untracked copy — so refuse up front
    // rather than leave a half-built lane that cannot come home.
    if (git('check-ignore -q .keel/state.json', dir).code !== 0) {
      git(`worktree remove ${JSON.stringify(dir)} --force`, cfg.root);
      git(`branch -D ${branch}`, cfg.root);
      return { ok: false, out: '.keel/state.json is not in .gitignore, so the lane would commit its own state and break the merge back.\nRun `keel upgrade --write` to add the missing ignore lines, then start the lane again.' };
    }

    const background = args.includes('--background');
    // Hand the lane its own state: the same flow and spec, but only its ACs and its own lane.
    // Without this the worktree starts empty and the lane's first `keel state red-done` has no
    // current AC to check.
    st.write({ root: dir }, Object.assign({}, st.EMPTY, {
      flow: parent.flow, size: parent.size, spec: parent.spec, branch,
      lane: name, phase: 'red', current: mine[0],
      acs: Object.fromEntries(mine.map((id) => [id, Object.assign({}, parent.acs[id], { status: 'todo' })])),
      gates: { mode: parent.gates.mode, bug_gates: parent.gates.bug_gates,
        skipped: background ? { [name]: 'rest-of-lane' } : {}, log: [] },
    }));
    // And take them off this session's board until the lane merges back.
    st.update(cfg, (s) => {
      s.lanes = s.lanes || {};
      s.lanes[name] = { branch, dir, acs: mine, mode: background ? 'background' : 'interactive', status: 'open' };
      for (const id of mine) s.acs[id].status = 'lane';
    });
    return { ok: true, out: [`lane ${name} started`, `  worktree: ${dir}`, `  branch:   ${branch}`,
      ...
      background ? '  gates are skipped in this lane; automatic checks still run' : '  open a second terminal there and run /keel:feature --lane ' + name].join('\n') };
  }
  if (sub === 'merge') {
    const s = st.read(cfg);
    const info = (s.lanes || {})[name];
    if (!info) return { ok: false, out: `no lane "${name}" recorded` };
    // Read the lane's own state before the merge: a failed merge returns early and leaves the
    // worktree standing, and a successful one removes it.
    const laneState = fs.existsSync(path.join(info.dir, '.keel', 'state.json')) ? st.read({ root: info.dir }) : null;
    const open = laneState ? st.acList(laneState).filter((id) => laneState.acs[id].status !== 'done') : [];
    if (open.length && !args.includes('--force')) {
      return { ok: false, out: `lane ${name} has not finished ${open.join(', ')}. Let it finish, or merge what is done with --force.` };
    }
    ...
    git(`branch -d ${info.branch}`, cfg.root);   // merged, so a safe delete; frees the name for the next lane
    st.update(cfg, (s2) => {
      // Fold the lane's bookkeeping back, so trace and the PR body can see its work at all.
      for (const id of st.acList(laneState || { acs: {} })) s2.acs[id] = laneState.acs[id];
      for (const l of (laneState && laneState.gates.log) || []) s2.gates.log.push(`[${name}] ${l}`);
      if (laneState && laneState.gates.skipped[name]) s2.gates.skipped[name] = laneState.gates.skipped[name];
      if (s2.lanes && s2.lanes[name]) s2.lanes[name].status = 'merged';
    });
    const done = laneState ? st.acList(laneState).filter((id) => laneState.acs[id].status === 'done') : [];
    return { ok: true, out: [`lane ${name} merged and its worktree removed`,
      done.length ? `  finished: ${done.join(', ')}` : null,
      open.length ? `  still open, now back on this session's board: ${open.join(', ')}` : null].filter(Boolean).join('\n') };
  }
  const s = st.read(cfg);
  const lanes = s.lanes || {};
...
function prBody(cfg, state) {
  ...
  return [
    ...
    // Ship steps the human turned off at the opening question. Kept separate from skipped
    // *gates*, because they mean different things: a skipped gate is a review nobody did, a
    // skipped ship step is a check that never ran — and in the deferred band, one whose push
    // gate is still outstanding.
    (state.ship_skipped || []).length ? '## Ship steps skipped\n\n'
      + state.ship_skipped.map((s) => `- ${s.step}: ${s.reason || 'no reason given'}`).join('\n') + '\n' : '',
